...
| `proodos26-9` | AM σήμα στο χρόνο και στη συχνότητα: tone modulation | Draw AM waveform AND spectrum for single-tone message (fc=500Hz, fm=1Hz); write x_AM, compute X_AM(f) | `am/conventional` §4 "Φάσμα του AM σήματος" | `am/conventional` §1 "Η εξίσωση Conventional AM" | ✅ CLOSED — viz-rework done; embeds AMSignalViz + AMSpectrumViz + OvermodulationPhaseReversalViz (inbox/272) |
| `proodos26-11` | USSB δύο σημάτων: φάσματα baseband και διαμορφωμένων | Draw baseband spectra (sinc(Wt) → rect; sinc²(Wt) → tri) then USSB-modulated spectra keeping only upper sideband | `am/ssb` §2c "Απόδειξη — γιατί το m·cos − m̂·sin δίνει μόνο USB" | `am/ssb` §3 "USSB-AM vs LSSB-AM — δύο επιλογές, ίδια πληροφορία"; `foundations/fourier-transform` (fourier-pair-rect, fourier-pair-tri) | ✅ CLOSED — viz-rework done; embeds FdmCanonicalProblemViz (mBW=0.5, kBW=1, kShape='triangle', USSB) — first live triangle wire + full DRAW+TEACH rework (inbox/300) |
...
| `jan26-th2-7` | AM σχεδίαση χρόνου + φάσματος | Draw overmodulated AM waveform (μ=2>1, c=cos(20πt), m=2sin(2πt); phase reversals visible) AND spectrum (carrier ± 1Hz sidebands with imaginary-weight from sin) | `am/conventional` §4 "Φάσμα του AM σήματος" | `am/conventional` §3 "Υπερδιαμόρφωση (overmodulation) — τι σπάει και γιατί" (time-domain waveform part); §2 (μ = Am/Ac = 2) | ✅ CLOSED — viz-rework done; embeds OvermodulationPhaseReversalViz (time) + AMSpectrumViz (spectrum); three impulse-pairs ±9/±10/±11 Hz, all height 1/2, BW 2 Hz (this step) |
| `jan26-th2-8` | DSB-SC με sinc message | Draw DSB-SC spectrum: sinc(2Wt) message → rect(f/2W) shifts to ±fc with NO carrier impulse | `am/dsb-sc` §3 "Φάσμα — μόνο sidebands" | `am/dsb-sc` §1 "Από Conventional AM στο DSB-SC" (signal form); `foundations/fourier-transform` (fourier-pair-rect: sinc ↔ rect) | ✅ CLOSED — viz-rework done (`FdmCanonicalProblemViz numChannels={1} initialMod="dsb" mBW={1}` single-channel DSB-SC: M(f) rect half-BW W → bands [f_c−W, f_c+W] at ±f_c, NO carrier impulse, BW 2W) + full DRAW+TEACH rework; centerpiece = why DSB-SC has no carrier line; faithful statement restored (this step) |
| `jan26-th3-mux` | AM-USSB Multiplexing — sinc + Π σε δύο φέροντα | Draw baseband spectra + USSB-modulated spectra for sinc(2Wt) on f₁=100kHz and Π(4Wt) on f₂=1MHz; draw multiplexed G(f) | `am/multiplexing` §4 "Η canonical εξεταστική άσκηση" | `am/ssb` §5 "Bandwidth και ισχύς" (B_SSB = W); `foundations/fourier-transform` (fourier-pair-rect) | ✅ CLOSED — viz-rework done (`FdmCanonicalProblemViz kBW={4}` USSB, commit 7846609) + full DRAW+TEACH rework (sync, this step) |
| `jun25-th2` | AM Multiplexing — sinc(Wt) DSB-SC + sinc(6Wt) DSB | Mixed FDM: DSB-SC + conventional AM channels; write signal forms, draw per-channel and combined spectra, derive non-overlap condition for n channels | `am/multiplexing` §4 "Η canonical εξεταστική άσκηση" | `am/multiplexing` §3 "Συνθήκη μη-επικάλυψης ανά σχήμα διαμόρφωσης"; `am/dsb-sc` §3 "Φάσμα — μόνο sidebands"; `am/conventional` §4 "Φάσμα"; `foundations/fourier-transform` | ✅ CLOSED — viz-rework done (`FdmCanonicalProblemViz` mixed DSB-SC + συμβατικό AM, carrier impulse); full DRAW+TEACH rework, non-overlap Δf≥7W/2 (this step) |
...
| `pa25-th2-2` | Σχεδίαση AM σήματος cos(8πt) με 2sin(2πt) | Draw AM waveform; μ = Am/Ac = 2 > 1 → overmodulation; identify envelope and phase reversals | `am/conventional` §3 "Υπερδιαμόρφωση (overmodulation) — τι σπάει και γιατί" | `am/conventional` §2 "Modulation index μ" (μ = 2 > 1 calculation) | ✅ CLOSED — viz-rework done (repeatGroup 'am-draw-cos8pi' with `pb25-th2-2`); embeds OvermodulationPhaseReversalViz (inbox/275) |
...
| `pa25-th3-mux` | AM-USSB Multiplexing — sinc(2Wt) + Π(4Wt) | USSB FDM: draw baseband and modulated spectra, derive non-overlap condition (f₂ ≥ f₁ + W), draw G(f) | `am/multiplexing` §4 "Η canonical εξεταστική άσκηση" | `am/multiplexing` §3 "Συνθήκη μη-επικάλυψης ανά σχήμα διαμόρφωσης"; `am/ssb` §5 "Bandwidth και ισχύς"; `foundations/fourier-transform` | ✅ CLOSED — viz-rework done (`FdmCanonicalProblemViz kBW={4}` USSB, commit f4378fc) + full DRAW+TEACH rework (sync, this step) |
...
| `pb25-th2-2` | AM σχεδίαση cos(8πt) με 2sin(2πt) | Same as pa25-th2-2 (repeatGroup 'am-draw-cos8pi'): draw overmodulated AM waveform; μ = 2 > 1; phase reversals | `am/conventional` §3 "Υπερδιαμόρφωση (overmodulation) — τι σπάει και γιατί" | `am/conventional` §2 "Modulation index μ" (μ = 2 > 1) | ✅ CLOSED — viz-rework done (repeatGroup 'am-draw-cos8pi' with `pa25-th2-2`); embeds OvermodulationPhaseReversalViz (inbox/275) |
...
| --- | --- | --- | --- |
| §2 "Το AM concept — info στο πλάτος του carrier" | `jan26-th2-6`, `pa25-th2-1` — **hot for 2** | `sept25-th1-1` (high-level motivation for AM) | Introductory concept section; no standalone must-learn formula; am-signal §6B weight=17 — the formula is TAUGHT in am/conventional §1, not here.  jan26-th2-6 + pa25-th2-1 are topic:'modulation' «λόγοι διαμόρφωσης» (repeatGroup:'why-modulate') exercises excluded from the original §3 topic:'am'-only sweep; see §7.F + §7.G G1 |
...
- ✅ `proodos26-9` — DONE (viz-rework; AMSignalViz + AMSpectrumViz + OvermodulationPhaseReversalViz, inbox/272)
- `sept25-th1-5` — draw two-tone AM spectrum → text-only (inbox/074)
- ✅ `jan26-th2-7` — DONE (viz-rework; OvermodulationPhaseReversalViz + AMSpectrumViz, this step)
- ✅ `pb25-th2-2` — DONE (viz-rework, repeatGroup 'am-draw-cos8pi'; OvermodulationPhaseReversalViz, inbox/275)
...
- ✅ `jan26-th2-8` — DONE (viz-rework: `FdmCanonicalProblemViz numChannels={1} initialMod="dsb" mBW={1}` single-channel DSB-SC + full DRAW+TEACH rework; centerpiece = why DSB-SC has NO carrier line; faithful statement restored) — was: text-only (inbox/081)
...
- ✅ `proodos26-11` — DONE (viz-rework: `FdmCanonicalProblemViz mBW={0.5} kBW={1} kShape='triangle'` USSB + full DRAW+TEACH rework; **first live `kShape='triangle'` wire**; centerpiece = how a baseband shape maps to its USSB lobe (sinc→rect W/2, sinc²→triangle W; one sideband survives, peak at carrier)) — was: static SVG only (inbox/080; inbox/300)
...
- ✅ `jun25-th2` — DONE (viz-rework: `FdmCanonicalProblemViz mBW={0.5} kBW={3} kShape="rect" kMod="am-conventional" initialMod="dsb"` wired — mixed DSB-SC (m) + συμβατικό AM (k, carrier impulse) FDM + full DRAW+TEACH rework; centerpiece = why conventional-AM shows a carrier line & DSB-SC doesn't + unequal-BW non-overlap built from actual bands → Δf≥7W/2, n≥1+7W/(2f₁); honest energy = ∞ from the carrier, info-energy 7/12W; faithful statement restored; BPF-Carson latent error fixed) — was: text-only (inbox/078; inbox/081)
