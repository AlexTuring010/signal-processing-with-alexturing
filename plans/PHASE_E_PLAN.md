...
- ☐ **E.1 — Inline `<ExamProblem>` citations**:
  ...
  - 17 per-lecture sub-tasks (rich-pool first per pre-flight recommendation):
    ...
    - ☑ **L10** (done 2026-05-25 — 3 inline citations at the natural pattern moments of the «Σωροί & Ξένα Σύνολα» lecture; **no Flame chips** — bank has zero 2024/2025 L10 entries, legitimate sparse-pool outcome per L04/L06/L08 precedent (the lecture's 6% exam-weight matches). `front-set-10-ask5` (Φροντιστηριακό #10 Άσκ. 5 — max-heap sift-down after key-decrease, medium) right after the Heapify-down `</Algorithm>` close, before `<BinaryHeapAnimator />` — the lecture's own «μικρότερο παιδί» trap callout immediately above sets up the chip's max-heap mirror («στο max-heap διαλέγουμε το μεγαλύτερο»); pattern cue authored as: «δοσμένος σωρός + αλλαγή τιμής σε μικρότερη + RA + αναδρομική σχέση + εφαρμογή» = heapify-down ντυμένο ως 4 υποερωτήματα, με το concrete στιγμιότυπο του φροντιστηρίου A=[16,14,10,8,10,9,3,2,4,7], 14→13 → 0 ανταλλαγές, 14→6 → 2 διαδοχικές βυθίσεις (κάτω από 10, μετά κάτω από 7, σταματά σε φύλλο), και Σήμα στην εκφώνηση «μικρότερη τιμή / δομή σωρού / αναδρομικός αλγόριθμος που διατηρεί τη δομή». `pt6-th3` (Σεπτέμβριος 2023 Θ.3 — Master Theorem + heapify-down, weight **25%**, **the headline L10 chip**, medium) right after `<BinaryHeapAnimator />`, before «### Ουρά προτεραιότητας» — the chip packages the same sift-down with a Master Theorem in front, exemplifying «η αναδρομή S(n)=S(2n/3)+Θ(1) είναι ακριβώς η ανάλυση του heapify-down»; pattern cue authored with the **non-obvious 2n/3** detail (NOT n/2 — ισοσταθμισμένο δυαδικό δέντρο επιτρέπει στο ένα υποδέντρο να είναι σχεδόν διπλάσιο του άλλου γιατί το τελευταίο επίπεδο γεμίζει από αριστερά), the full Master Theorem case-2 derivation ($n^{\log_{3/2} 1} = n^0 = 1$ ίσο με f → Θ(log n)), the «**log n = ύψος δέντρου**» punchline («δεν είναι αλγεβρικό τέχνασμα — είναι κυριολεκτικά το ύψος του σωρού»), and the generalisation «κάθε σωρός-πράξη έχει αναδρομή S(n) = S(c·n) + Θ(1) με 0 < c < 1 → πάντα case 2». `front-set-5-ask10` (Φροντιστηριακό #5 Άσκ. 10 — Πυθαγόρεια τετράδα O(n²) via hash «meet in the middle», hard) right after the Heap RecallCard, before «## Μέρος Β' — Ξένα Σύνολα» — the content-gap chip per the L10 ExamRadar's «high-likelihood» hash-table item; honestly framed with a Greek bridge that acknowledges «ο Στέλιος δεν ξεδιπλώνει αναλυτικά τους πίνακες κατακερματισμού εδώ», but the exams treat them as L10-υλικό. Pattern cue authored as the **meet-in-the-middle** recipe: μετάφραση πολυώνυμης εξίσωσης σε «αριστερή πλευρά = δεξιά πλευρά» με k/2 στοιχεία η καθεμία (a²+b² = d²-c²), Πέρασμα 1 χτίζει hash set των αριστερών (O(n²) ζεύγη), Πέρασμα 2 ψάχνει τις δεξιές (O(n²) lookups), συνολικά O(n^(k/2))=O(n²) αντί O(n^k)=O(n⁴); + the **memory-trap** explanation (όταν οι τιμές κυμαίνονται σε «{1, …, n⁴}», ο άμεσος πίνακας πέφτει σε memory πριν πέσει σε time — γι' αυτό hash, όχι direct addressing); + Σήμα στην εκφώνηση triplet (i) εξίσωση που χωρίζεται + (ii) ζητούμενος χρόνος = √(αφελής) + (iii) η λέξη **«αναμενόμενος»** ως τεχνική απαίτηση («χωρίς αυτή τη λέξη η απάντηση είναι λάθος»). Three distinct pattern flavors clustered across Part A: (a) stand-alone heapify-down (frontistirio), (b) Master-Theorem-as-heapify-analysis (Σεπτ 2023 25%), (c) meet-in-the-middle hash (content-gap acknowledged). Two minimal Greek interstitial sentences bridge chips 1↔2 and chip 3 («Αυτή ακριβώς η συνταγή … εμφανίζεται αυτούσια ως αυτόνομο πρόβλημα …»; «Και ολόκληρο Θέμα στις εξετάσεις πακετάρει την ίδια βύθιση μαζί με ένα Master Theorem μπροστά …»; «Πριν περάσουμε στη δεύτερη δομή, μια παρένθεση που η εξέταση κάνει συχνά …»). Part B (Union-Find) intentionally has zero chips — the bank has no Union-Find-tagged entries (sparse-pool outcome) and the «Πώς το union-find επιταχύνει τον Kruskal» section already cross-refs L09. All bottom surfaces unchanged: the existing 6-item `<ExamRadar />` already lists 2 of the 3 chip targets (pt6-th3 under heapify-up/down 'high'; front-set-5-ask10 + front-set-5-ask11 under hash-table pair-sum 'high') — chip + radar are complementary surfaces per the L02/L03/L09/L14/L15/L01/L04/L06/L08 precedent; front-set-10-ask5 is NOT in the existing radar (it overlaps with pt6-th3 as the frontistirio variant — that's why it's a chip, not a radar item). Server component, zero client JS shipped — L10 bundle stays 158 B / 676 kB (unchanged from B7.5 close). typecheck+lint+test+build all pass; tests still 20/20. One typo fix during authoring («monοπάτι» → «μονοπάτι» — mixed Latin/Greek script). See [[phase-e1-L10-citations]])
    - ⏸ **L11, L12, L13, L17 — PAUSED 2026-05-24** per user direction («quality-audit tasks are of more importance, so they should be earlier»). Resume via «E.1 (resumed)» below, after E.4.5 + E.4.6 close.
    - ☐ L05 ☐ L07 ☐ L16 — documented-and-skipped per Phase D precedent (sparse/empty pool)
- ☑ **E.4.5 — «Νιώσε»-visual audit & retrofit across the problem bank** (PROMOTED 2026-05-24 — took precedence over remaining E.1 and everything below; CLOSED 2026-05-25 with E.4.5.B re-audit). One audit pass → 4 definite gaps + 2 borderlines → Chunk A retrofitted (4 visuals, commit `21d88db`) → Chunk B re-audited and both borderlines confirmed-skip. Binding standard now recorded: «every transcribed problem whose statement describes a concrete scenario must open its solution with a good-quality illustrative anchor of the scenario as described, before any abstract modeling. Per-problem static-vs-interactive — never default-static when motion is the teaching surface.» Future E.5 transcriptions adopt this from the start.
  ...
  - ☑ **E.4.5.B — Chunk B re-audit** (done 2026-05-25 — both borderlines confirmed-skip on second read. `front-set-6-ask4` (Γιώργος καθαριστήριο): `<LaundryFlowShop />`'s Gantt already encodes the «1 serial + N parallel» dichotomy — top row labelled «Έλεγχος» renders the five `s_i` serial-bottleneck blocks back-to-back, and the five lower rows (each labelled «X πλύση») render the `p_i` parallel rectangles starting exactly when each item's check ends, widths to scale; a literal cleaner-shop SVG (Γιώργος + check station + N washer pictograms) would re-skin those exact relationships without adding teaching content, and the exam expects the student to draw the Gantt, not a shop floor plan. `front-set-7-ask9` (πάρτι Alice): `<PartyDegreeFilter />` already opens with 10 numbered vertices (= υποψήφιοι καλεσμένοι) + 19 undirected edges (= γνωριμίες) laid out as a 6-cycle core + social-butterfly (deg 9) + two outliers, with live in-graph friend-counters on each guest and the peel animation as the storyline; an «Alice's named-guest-list» tableau would just rename the existing vertex ids without changing the picture or surfacing any new motion. Documented in `plans/E_VISUAL_AUDIT.md` § 4.1 + § 4.2 + § 7. No code/visual changes per § 5's «if both stay skipped, Chunk B is folded into the wrap-up of Chunk A's commit» — Chunk A already shipped in commit `21d88db`. See [[phase-e45-chunk-b-skip]])
- ☑ **E.4.6 — Generic edge-routing utility across all graph vizzes** (PROMOTED 2026-05-24 — took precedence over remaining E.1 and everything below; CLOSED 2026-05-25 with B7.5). `routeEdge()` shipped + per-viz retrofit complete across all 7 B-chunks (B1..B7.5). Every in-scope graph viz on the site now routes through collision-aware edge geometry by construction; final carve-out collection of hand-tuned curves is 7 documented exceptions where the curve IS the teaching surface. See sub-tasks for details.
  ...
  - ☑ chunk B7 in `plans/E_EDGE_ROUTING_AUDIT.md` (done 2026-05-25 — final retrofit chunk; re-split into 5 sub-chunks B7.1..B7.5 along «shares-a-layout-shape» lines, all 5 ticked below):
...
- ☑ **E.1 (resumed) — L11, L12, L13, L17 per-lecture inline citations** (un-paused 2026-05-25 with L10 closed, CLOSED 2026-05-25 with L17; same one-lecture-per-turn cadence as the completed L09…L08 + L10 chips. All 14 in-scope L's now done; L05/L07/L16 remain documented-and-skipped per Phase D precedent.)
  - ☑ **L13** (done 2026-05-25 — 3 inline citations at L13's three natural Greedy III / Huffman pattern moments, **one Flame** (pt3-th3 Ιούνιος 2024 30%, the only L13-tagged Flame the bank has); the first two clustered around the «καθαρή εξεταστική μορφή του αλγορίθμου» surface (mirror of L12's clustered chips lesson), the third standing alone at the ThinkingPattern «αυτή η συνταγή γενικεύεται» surface. `front-set-6-ask7` (Φροντιστηριακό Σετ #6 Άσκ. 7 — Κωδικοποίηση Huffman ΚΑΣΤΑΝΑΣ, medium) right after the ### Πολυπλοκότητα section + the `$$T(n) = O(n \log n).$$` close, before «## Γιατί είναι βέλτιστη» — the canonical 4-υποερωτημάτων exam form (δέντρο/πίνακας/κωδικοποίηση/αποκωδικοποίηση), packaged with the mechanical 4-step recipe (ταξινόμηση→`n-1` συγχωνεύσεις · μονοπάτι ρίζα→φύλλο με `0`=αριστερό/`1`=δεξί · αντικατάσταση χαρακτήρων · ανάγνωση bit-bit με μονοσήμαντη επιστροφή στη ρίζα κάθε φορά που φτάνεις σε φύλλο), the «τι κάνεις με τα bits που μένουν στο τέλος» edge case, and a 1-line ορθότητας έλεγχος «κανένας κώδικας πρόθεμα άλλου ⟺ χαρακτήρες μόνο σε φύλλα». `front-set-7-ask1` (Φροντιστηριακό Σετ #7 Άσκ. 1 — Ένωση n ράβδων χρυσού με ελάχιστο κόστος, medium) clustered immediately after with a one-line bridge naming the disguise («όχι χαρακτήρες, αλλά ράβδοι χρυσού που ενώνονται 2-2· όχι "μήκος κώδικα", αλλά "πόσες φορές πληρώνεται κάθε αρχικό βάρος"») — the mathematical identification spelled out ($w_i$ πληρώνεται «βάθος$(i)$» φορές → $\sum_i w_i \cdot d(i)$ = γράμμα προς γράμμα το $\sum_x f_x \cdot |c(x)|$ του Huffman με $w_i \leftrightarrow f_x$ και «βάθος» $\leftrightarrow$ «μήκος κώδικα»), the min-heap υλοποίηση spelled out (αρχικός σωρός $O(n)$ + $n-1$ συγχωνεύσεις × $O(\log n)$ = $O(n\log n)$), the παρόμοιες παραλλαγές list (αρχεία/σχοινιά/k-merge — «κάθε ιστορία όπου δύο γίνονται ένα και πληρώνουν τη μάζα τους κάθε φορά»), the αντι-άπληστος παγίδα («δύο τυχαίες» / «μικρότερη + μεγαλύτερη» → βαριές πέφτουν βαθιά), AND the «δεν ζητείται απόδειξη ορθότητας → τουλάχιστον ονοματίσε ως Huffman σε άλλη ορολογία» μισός-βαθμός tip. `pt3-th3` (Ιούνιος 2024 · Θέμα 3 — Τέλειο ταίριασμα σε δέντρο, weight **30%**, **Flame**, medium) right after `</ThinkingPattern>`, before `<ExamRadar>` — the «αναγκαστική κίνηση είναι δωρεάν» generalization beyond Huffman to a fresh 2024 problem on a different greedy pattern, with the «κάθε βέλτιστη λύση υποχρεούται» = αυστηρότερη εκδοχή του Λήμματος 2 («κάποια βέλτιστη λύση τα έχει αδέλφια») framing made explicit, the algorithm in one sentence (διάλεξε φύλλο · κλείδωσε την ακμή · σβήσε ζεύγος · συνέχισε), the «αν εμφανιστεί απομονωμένη κορυφή, δεν έχει τέλειο ταίριασμα» edge case + WHY (η κορυφή δεν έχει πια ακμή για να καλυφθεί), the επιχείρημα ανταλλαγής + επαγωγή σε $n - 2$ κορυφές, the $O(n)$ + FIFO ουρά τρεχόντων φύλλων υλοποίηση, and the explicit «Huffman χαλαρή αναγκαιότητα vs εδώ σφιχτή» Σύγκριση. **One Flame chip** (out of 1 available — sparse-pool outcome per L11/L10/L08/L06/L04 precedent; the bank's only L13-Flame entry is `pt3-th3`; the other 4 L13 entries are all frontistirio-2023-24). **3 of 5 bank entries cited** — the 2 fractional-knapsack entries (`front-set-7-ask12`, `front-set-10-ask13`) intentionally NOT chipped because they're about a different greedy concept than Huffman (the only-Greedy-III lecture topic) and the existing `<ExamRadar>` already lists `front-set-7-ask12` as 'high' likelihood; adding them as 4th/5th chips would dilute technique cohesion. **Clustered chips pattern applied (L12 mirror).** Chip 1 + chip 2 share «execute Huffman, possibly disguised — same algorithm, two surface forms»; chip 3 stands alone at the «same TECHNIQUE, different greedy problem» surface. The clustering is pedagogically warranted because (a) both `front-set-6-ask7` and `front-set-7-ask1` are direct-execution exam forms of the just-shown algorithm, (b) the bank's `front-set-7-ask1` solution explicitly names this disguise pattern («είναι Huffman, χρειάζεσαι μόνο min-heap και τη φράση «δύο ελαφρύτερες»»). All bottom surfaces unchanged: the existing 8-item `<ExamRadar />` already lists all 3 chip targets (`front-set-6-ask7` appears under 3 items; `front-set-7-ask1` under «Huffman σε άλλη ορολογία — ένωση χρυσού / σχοινιών / αρχείων / k-merge»; `pt3-th3` under both «Απόδειξη βελτιστότητας του Huffman — επιχείρημα ανταλλαγής φύλλων και επαγωγή» and «Άπληστος σε δέντρο μέσω αναγκαστικών κινήσεων από τα φύλλα»). Server component, zero client JS shipped — L13 bundle 157 B / 676 kB (1 B rounding-noise delta from the L12 close 158 B; First Load unchanged). typecheck+lint+test+build all pass; tests still 20/20. See [[phase-e1-L13-citations]])
  - ☑ **L11** (done 2026-05-25 — 3 inline citations at the three natural Greedy I pattern moments, one per technique the lecture teaches: `pt2-th2-3` (Σεπτέμβριος 2025 Θ.2.3 — «Άπληστα ρέστα αποτυγχάνει», weight 10%, **Flame**) right after the «Ένα αντιπαράδειγμα αρκεί» warning Callout, before «### Το κριτήριο που δουλεύει» — the canonical «πρώτη κίνηση = αντιπαράδειγμα» recipe applied to a fresh 2025 problem (coin change with {1,10,25}) with the «η ορθότητα ζει στις αξίες, όχι στον κανόνα» double-look against {1,5,10,25} dollar system; `front-set-6-ask6` (Φροντιστηριακό Σετ #6 Άσκ. 6 — gas stations) right after the «Η τεχνική: ο άπληστος προηγείται» Callout, before the first RecallCard — mechanical-repetition of the just-shown «greedy stays ahead» proof on a linear-horizon problem (induction `g_k ≥ o_k`, base from «πιο μακρινός μέσα στην εμβέλεια», induction step from «g_k + n ≥ o_k + n»), with the «γραμμικός ορίζοντας + πόρος που καίγεται + σταθερό κόστος ανά κίνηση» recognition signal that the same recipe rewrites several variants (στάσεις, αλλαγές γραμμής, restart διεργασιών); `front-set-6-ask8` (Φροντιστηριακό Σετ #6 Άσκ. 8 — άπληστος χρωματισμός + ταξί) right after the «δομικό κάτω φράγμα» intuition Callout, before the second RecallCard — packages TWO greedy algorithms with opposite verdicts and uses the L10 «content-gap chip» pattern: part (α) general graph coloring is **NP-hard** and «η σειρά είναι ο αλγόριθμος» («ο Στέλιος δεν εμβαθύνει στους άπληστους χρωματισμούς εδώ, αλλά η εξέταση τους θεωρεί υλικό του L11»), while part (β) ταξί = γράφος επικάλυψης = **γράφημα διαστημάτων** turns the same algorithm into the optimum via «ελάχιστα ταξί = βάθος = χρωματικός αριθμός» triple-identity that the same depth-bound argument from a moment ago proves verbatim. **One Flame chip** (out of 1 available — `pt2-th2-3` is the only recent-paper L11 entry the bank has; all other L11-tagged entries are frontistirio). Three distinct technique flavors at three distinct lecture-arc anchors: (a) counterexample-first (post-«αντιπαράδειγμα αρκεί»), (b) «ο άπληστος προηγείται» on a fresh linear-horizon problem (post-«η τεχνική»), (c) structural-lower-bound + content-gap framing on coloring (post-«δομικό κάτω φράγμα»). Three minimal Greek interstitial bridges frame the chips («Αυτή η αντανακλαστική κίνηση … έχει τη δική της εξεταστική μορφή σε εντελώς διαφορετικό σκηνικό. Όχι σε διαστήματα: σε ρέστα.», «Και η ίδια τεχνική — "ο άπληστος δεν μένει ποτέ πίσω" — έχει τη δική της εξεταστική μορφή σε φρέσκο πρόβλημα: αυτή τη φορά σε γραμμικό δρόμο.», «Και η ίδια "βάθος = ελάχιστος πόρος" λογική επιστρέφει σε ένα φροντιστηριακό θέμα που πακετάρει δύο άπληστα κοντά-κοντά — έναν που η σειρά τον σπάει, και έναν που η σειρά τον κάνει βέλτιστο ακριβώς επειδή το γράφημα τυχαίνει να είναι γράφημα διαστημάτων.»). All bottom surfaces unchanged: the existing 8-item `<ExamRadar />` already lists all three chip targets — `pt2-th2-3` under «Άπληστα ρέστα — πότε δουλεύει και πότε αποτυγχάνει ο κανόνας "μεγαλύτερο κέρμα"», `front-set-6-ask6` under «Απόδειξη "ο άπληστος προηγείται" για χρονοπρογραμματισμό ή παραλλαγή του», `front-set-6-ask8` under both «Διαμέριση διαστημάτων — το επιχείρημα του βάθους ως κάτω φράγμα» and «Άπληστος χρωματισμός γραφημάτων — γιατί η σειρά εξέτασης μετράει» — chip + radar are complementary surfaces per the L02/L03/L09/L14/L15/L01/L04/L06/L08/L10 precedent. Server component, zero client JS shipped — L11 bundle stays 158 B / 676 kB (byte-identical to the L10 close). typecheck+lint+test+build all pass; tests still 20/20. See [[phase-e1-L11-citations]])
  - ☑ **L17** (done 2026-05-25 — 3 inline citations at L17's three natural DP IV pattern moments, **two Flame chips of 2 available** (mirror of L12's rich-pool outcome, but spread across distinct surfaces rather than clustered): `front-set-8-ask1` (Φροντιστηριακό Σετ #8 · Άσκ. 1 — Μέσο κόστος όλων των μονοπατιών σε DAG, medium) right after `<TreeIndependentSet />`, before «## Συντομότερες διαδρομές με αρνητικά βάρη» — the «δύο τιμές ανά κορυφή» recipe from the πάρτι problem generalised beyond trees to DAGs with `count[x]` + `sum[x]` filling in lock-step, with the explicit double-recurrence spelled out as a BlockMath, the mathematical identification of «κάθε ακμή (x, y) προσφέρεται σε ΟΛΑ τα `count[y]` μονοπάτια» reading-aloud, the reverse-topological-order recipe, the `sum[s]/count[s]` answer, the Θ(V+E) complexity, the «μέσο μήκος συντομότερων μονοπατιών / πλήθος βέλτιστων λύσεων / αναμενόμενη τιμή τυχαίας πορείας σε DAG» variants list, and the Σήμα-στην-εκφώνηση «μέσος / μέση / αναμενόμενη + όλα τα μονοπάτια / λύσεις / υποακολουθίες» + παγίδα «αν αρκεστείς σε μία αναδρομή, η σύνθεση σπάει σιωπηλά». `pt4-th1-q3` (Σεπτέμβριος 2024 · Θέμα 1.3 — Σ/Λ «ο Bellman-Ford είναι άπληστος;», weight **4%**, **Flame**, easy) right after `<BellmanFordAnimator />`, before «### Βελτιώσεις» — the «κλείδωμα ή πίνακας;» διαγνωστικό operationalised: άπληστος ⇔ μία αμετάκλητη απόφαση ανά βήμα + locked set (Dijkstra/Prim/Kruskal με extract-min-+-κλείδωμα, συνήθως O(m log n) ή O(m·α(n))) vs DP ⇔ πίνακας με υποπροβλήματα ταξινομημένα σε μονότονη παράμετρο, νέα γραμμή διαβάζει την προηγούμενη (Bellman-Ford με M[i,v] παραμετροποιημένο με «πλήθος ακμών», καμία κορυφή δεν κλειδώνει, Θ(μέγεθος πίνακα) = Θ(mn)); the «και οι δύο χαλαρώνουν ακμές με την ίδια γραμμή min(M[v], M[u]+ℓ)» παγίδα ρητά ονομασμένη («η πραγματική διαφορά δεν είναι στο ΤΙ κάνουν αλλά στο ΠΩΣ το θυμούνται»); the L17-specific «άλλοι DP-σε-γραφήματα» list (tree-IS με παραμετροποίηση υποδέντρα, shortest-path-σε-DAG με τοπολογική σειρά) AND «άλλοι άπληστοι σε γραφήματα» list (Dijkstra/Prim/Kruskal με extract+lock); και Σήμα-στην-εκφώνηση «Σ/Λ ή MCQ που ζητάει κατηγοριοποίηση + αλγόριθμος που και οι δύο σχολές διεκδικούν (Bellman-Ford η κλασική παγίδα)». `pt1-th2-a` (Ιούνιος 2025 · Θέμα 2.1 — Ανίχνευση αρνητικού κύκλου, weight **3%**, **Flame**, easy) right after the «Παραλλαγές» Callout, before the first `<RecallCard>` — the just-mentioned «αν ύστερα από n−1 γύρους κάτι ακόμα αλλάζει, υπάρχει αρνητικός κύκλος» bullet's exam-form: τρέξε n (όχι n−1) γύρους, αν στον γύρο n μειώθηκε ξανά τιμή → αρνητικός κύκλος προσβάσιμος προς το t; the WHY spelled out (χωρίς αρν. κύκλο η συντομότερη διαδρομή είναι απλή → ≤ n−1 ακμές → M[n-1,·] = M[n,·]· αν αντί γι' αυτό η M[n,·] αλλάζει, κορυφή «γλιστράει» σε κύκλο και μαζεύει «δωρεάν» μειώσεις)· complexity stays Θ(mn) («ένας γύρος παραπάνω δεν αλλάζει την τάξη»)· γιατί ΟΧΙ Dijkstra («σπάει από το πρώτο αρνητικό»)· και Σήμα-στην-εκφώνηση «αρνητικός κύκλος / ανίχνευση / έλεγχος ύπαρξης / αποφάσισε αν υπάρχει + κατευθυνόμενο γράφημα με ίσως αρνητικά βάρη» + «**Πρότυπο σκέψης — "έξτρα γύρος = ανιχνευτής"»** generalisation («κάθε φορά που DP αλγόριθμος εγγυάται σύγκλιση μετά από k γύρους, ένας (k+1)-οστός γύρος δίνει δωρεάν διαγνωστικό»). **Two Flame chips of 2 available**, but unlike L12 spread across THREE distinct teaching surfaces (post-TreeIS / post-BFAnimator / post-Παραλλαγές) rather than clustered — this is the **first "rich-pool, distinct-surfaces" outcome** in Phase E.1 (cf. L12's "rich-pool, clustered"). Three minimal Greek interstitial bridges frame the chips: «Πριν περάσουμε στις συντομότερες διαδρομές με αρνητικά βάρη, αξίζει να δούμε ότι αυτή ακριβώς η ιδέα — "μία τιμή ανά κορυφή δεν αρκεί" — επιστρέφει εκτός δέντρων…», «Αυτή ακριβώς η μηχανική — "νέα γραμμή του πίνακα διαβάζει ολόκληρη την προηγούμενη" — είναι η ζωτική διαφορά που τοποθετεί τον Bellman-Ford ξεκάθαρα στον δυναμικό προγραμματισμό, όχι στους άπληστους…», «Και η τρίτη βελτίωση παραπάνω — "αν ύστερα από n−1 γύρους κάτι ακόμα αλλάζει, υπάρχει αρνητικός κύκλος" — κρύβει ένα ολόκληρο εξεταστικό θέμα…». Three distinct pattern flavors at three distinct lecture-arc anchors: (a) double-recursion in DAG generalising A[v]/B[v] (post-TreeIS, frontistirio), (b) greedy-vs-DP categorisation diagnostic «κλείδωμα ή πίνακας;» (post-BFAnimator, Σεπτ 2024 Flame), (c) extra-round detector for negative cycles (post-Παραλλαγές, Ιούν 2025 Flame). **All three chip targets already listed in the existing 7-item `<ExamRadar />`** — `front-set-8-ask1` under «Διπλή αναδρομή σε DAG — count + sum» 'medium' with note; `pt4-th1-q3` under BOTH «Bellman-Ford — τρέξε τον» 'high' AND «Ταξινόμηση αλγορίθμων: άπληστος vs DP — διαγνωστικό "κλείδωμα ή πίνακας;"» 'high' with note; `pt1-th2-a` under «Αρνητικοί κύκλοι: γιατί καταργούν τη συντομότερη διαδρομή, και πώς εντοπίζονται» 'medium'. Chip + radar are complementary surfaces per the L02/L03/L09/L14/L15/L01/L04/L06/L08/L10/L11/L12/L13 precedent — chip = inline recognition at the lecture moment that introduces the technique, radar = bottom-page index for exam-prep scanning. Server component, zero client JS shipped — L17 bundle 158 B / 676 kB (byte-identical to the L12/L13 close). typecheck+lint+test+build all pass; tests still 20/20. **L17 closes E.1 (resumed) and E.1 end-to-end for the 14 in-scope L's; L05/L07/L16 remain documented-and-skipped per Phase D precedent (sparse/empty pool).** See [[phase-e1-L17-citations]])
  - ☑ **L12** (done 2026-05-25 — 3 inline citations at L12's three natural Greedy II pattern moments, two of them Flame and clustered around the «3 τεχνικές απόδειξης» meta-Callout to show the BREADTH of exchange argument across recent exams: `pt2-th4` (Σεπτέμβριος 2025 · Θέμα 4, weight 25%, **Flame**, hard — «μία μηχανή + ελαχιστοποίηση Σ fᵢ → SPT αύξον tᵢ») right after the EDF RecallCard, before «### Οι τρεις τεχνικές απόδειξης» — the canonical exchange-argument-on-a-different-objective recipe applied to a brand-new 2025 problem, with the mechanical proof spelled out (αντιμετάθεση δύο διαδοχικών εκτός-σειράς μειώνει το άθροισμα κατά $t_i - t_j > 0$· συνεισφορά του ζεύγους πέφτει από $2T + 2t_i + t_j$ σε $2T + 2t_j + t_i$) and the explicit «Σύγκρινε με τον EDF: ίδια τεχνική, αλλαγή αντικειμενικής max→sum → αλλαγή κλειδιού $d_i$→$t_i$· το EDF "δεν αυξάνει το max", το SPT "μειώνει το sum"» framing; `front-set-6-ask4` (Φροντιστηριακό #6 · Άσκ. 4, Γιώργος καθαριστήριο, medium) clustered immediately after `pt2-th4` with a one-line bridge that names the reversal — «δύο φάσεις (σειριακή στενωπός + παράλληλη ουρά που τρέχει στο παρασκήνιο μόλις τελειώσει η σειριακή ανά εργασία) + ελαχιστοποίηση makespan → LPT φθίνον $p_i$, ο κανόνας αγνοεί εντελώς τη σειριακή φάση», με το παγκόσμιο μάθημα «μία φάση + sum → αύξον $t_i$· δύο φάσεις + makespan → φθίνον $p_i$» ρητά ονοματισμένο και τη γρήγορη απόδειξη του makespan με $\Sigma = s_i + s_j$ (παλιό χειρότερο τοπικό = $\Sigma + p_j$· νέο = $\max(s_j + p_j, \Sigma + p_i) \le \Sigma + p_j$ επειδή $p_j > p_i$) — οι δύο chips μαζί ξεδιπλώνουν την ίδια τεχνική σε δύο διαφορετικά scenarios· `pt2-th2-2` (Σεπτέμβριος 2025 · Θέμα 2.2, weight 5%, **Flame**, easy — «σε ποιες κλάσεις δουλεύει σωστά η τοπολογική ταξινόμηση για κάθε γράφημα της κλάσης;») right after the proof of «τοπολογική διάταξη ⇔ DAG» (close of Μέρος 3), before «### Ο αλγόριθμος» — η εξεταστική εφαρμογή του «⇔ DAG» που μόλις απέδειξε, με τη μηχανική τρι-βηματη συνταγή (1. τι απαιτεί ο αλγόριθμος → κατευθυνόμενο+ακυκλικό· 2. η ιδιότητα της κλάσης τα εγγυάται;· 3. αν όχι, αντιπαράδειγμα με κύκλο αρκεί) και 4 ταξινομημένα παραδείγματα ✗βάρη ($A \to B \to C \to A$ με βάρη 3/1/4)·✗διμερές ($A_1 \to B_1 \to A_2 \to B_2 \to A_1$ άρτιος κύκλος)·✓δέντρο (κατευθυνόμενο/ριζωμένο ⇒ ειδική περίπτωση DAG)·✓DAG (εξορισμού), ακριβώς όπως το πραγματικό θέμα του Σεπτεμβρίου 2025, και με την «παγίδα διατύπωσης» («κάθε» ≠ «κάποια») ρητά υπογραμμισμένη. **Two Flame chips out of 2 available** — first lecture in Phase E.1 (resumed) to hit a rich pool (cf. sparse L04/L06/L08/L10/L11). **New placement pattern — clustered chips:** rather than spreading 3 chips across 3 distinct teaching surfaces (L09/L14/L02/L03/L15/L01/L11 standard) or 3+1 with content-gap (L10), L12 has only TWO teaching surfaces (Lateness / Topo) and the two lateness chips form a mini-cluster of «same technique, two recent exam variants» with a 1-line bridge between them that explicitly names the reversal SPT↔LPT — pedagogically warranted because both `pt2-th4` and `front-set-6-ask4` are exchange-argument problems and the bank's `pt2-th4` solution explicitly cross-references `front-set-6-ask4` («Σύγκρινε με την προηγούμενη άσκηση»). Bottom surfaces unchanged: the existing 8-item `<ExamRadar />` already lists all 3 chip targets (`pt2-th4` under «SPT αύξον $t_i$»· `front-set-6-ask4` under «Two-stage flow shop … φθίνον $p_i$ (LPT)»· `pt2-th2-2` under «Σε ποιες κλάσεις δουλεύει η τοπολογική ταξινόμηση»). Server component, zero client JS shipped — L12 bundle stays 158 B / 676 kB (byte-identical to the L11 close). typecheck+lint+test+build all pass; tests still 20/20. See [[phase-e1-L12-citations]])
- ☑ **E.2 — Pattern-pair catalogue** → `plans/E_PATTERN_PAIRS.md` (done 2026-05-25 — 37 pairs/triples documented across 14 lectures (target 12–25, vastly exceeded) + 29 solo fingerprints kept for radar; 30 definite + 7 strong-analogy pairs; 26 of 37 carry at least one 2024/2025 entry (Flame). Each pair has fingerprint name, ID list, recognition cue with Σήμα-στην-εκφώνηση + 3-step recipe form (mirroring E.1 convention), confidence score, and target lecture for `<ExamRadar />` + `<RelatedPair>` retrofit. Per-lecture pair density: L02 (7), L09 (6), L03 (6), L14 (5), L15 (4), L04+L08 (3 each), L01/L11/L12/L13/L16/L17 (1–2 each), L05/L06 (1), L07 (none). Headlines: P13.1 Huffman canonical ⇔ disguised (the user's «τραπεζικές κάρτες ⇔ αρχαίες πλάκες» canonical example realised in this bank as ΚΑΣΤΑΝΑΣ ⇔ n ράβδοι χρυσού); P9.8 D(MST/TSP/ST) ∈ NP cluster (5 papers); P15.1 0/1 σακίδιο literal ⇔ disguised (Σεπτ 2024 40%); P14.1 WIS literal across πλατφόρμα δόνησης ⇔ μαγνητικός τομογράφος; P14.4 recursion explosion vs DP (Ιουν 2022 35%); P4.1 majority element D&C (ύποπτη κάρτα ⇔ πλειοψηφικό στοιχείο — the closest live instance of the user's named example pattern). Doc also catalogues E.3 implications (per-lecture queue ordered by pair density) and E.5 transcription priorities (which solos most want twins). Research + document only — no code changes. See [[phase-e2-pattern-pairs]])
- ☐ **E.3 — `<RelatedPair>` + bidirectional surfacing** (per-lecture, depends on E.2):
  - ☑ **E.3.0 — `<RelatedPair>` component build** (done 2026-05-25 — server component, dual-mode banner: `definite` → «Ίδια άσκηση, άλλο όνομα» with `Link2` icon / `analogy` → «Ίδιο μοτίβο, άλλη γωνία» with `GitCompareArrows` icon; indigo→violet gradient styling that is distinct from the rose-tinted `<ExamProblem>` chip and from every existing `<Callout>` tone, so the student notices the block immediately; supports pairs of any size (2, 3, or more) via `pairs={[{id}, …]}` — each row deep-links to `/practice#exercise:<id>` and renders `SOURCE_LABELS[ex.source]` + `ex.problemNumber` + stripped title (reusing the bank-prefix-strip helper pattern from `ExamProblem`) + a Flame chip for 2024/2025 sources + the weight % chip when present; bottom cue card («Σήμα στην εκφώνηση» eyebrow over the recognition cue, white-tinted card-in-card so it reads as a separate signal from the pair listing); dev-mode `AlertTriangle` block for missing-id lookups (silent in production). Zero client JS shipped — `EXERCISES`/`SOURCE_LABELS`/`RECENT_SOURCES` resolved at build time. Registered in `mdx-components.tsx` alongside `<ExamProblem>`. typecheck + lint + test (20/20) + build all pass; bundle sizes unchanged — `/practice` 5.14 kB / 456 kB, `/practice/sose-to-eksamino` 8.82 kB / 453 kB, lectures 158 B / 676 kB (the +1 kB shift on practice/sose vs B7.5 baseline reflects the new icon imports but no measurable per-route impact). See [[phase-e3-component-build]])
  - Per-lecture retrofit (ordered by E.2 pair density — bidirectional embed inside both problems of every pair):
    - ☑ **L02** (done 2026-05-26 — 5 lecture-page `<RelatedPair>` blocks at the 5 multi-entry pattern moments (P2.1 ranking after `HierarchyRace`+«πρακτικός κανόνας»; P2.2 T/F identities after Ιδιότητες Example; P2.3 nested-loops after Πρακτικές ασκήσεις Example 4; P2.6 sub-log after Πολυωνυμικό-κυριαρχεί-σε-πολυλογαριθμικό Example; P2.7 ταυτότητα-πρώτα after existing `pt1-th1-q2` chip) + bidirectional embeds in 24 transcribed problem solutions (pt1-th1-q1/q2/q3, pt2-th1-q1/q2, pt4-th1-q2/q5, pt5-th1b, pt5-th2-a from past-exam; front-set-1-ask0/ask1/ask3, front-set-2-ask0/ask2/ask3/ask4/ask5/ask6/ask7, front-set-10-ask2, front-set-11-ask3, front-set-12-ask1/ask2, front-set-4-e0-ask6/thema4 from frontistirio — `front-set-11-ask1` skipped because untranscribed but referenced from lecture page + pt2-th1-q2 solution + front-set-1-ask3 solution as a deep-link). Solos P2.4/P2.5 surfaced ONLY via bidirectional embeds in pt4-th1-q5 + pt2-th1-q1 solutions as «analogy: Σ → κλειστός τύπος → Θ-από-ορισμό» — no standalone lecture-page block (both problems already inline-chipped with `<ExamProblem>`, RelatedPair would be redundant). Pair-pattern templated cues across each pair (consistent recognition signal for the student) with localised flavor per entry (each cue references the specific instance — Greek bridges name the disguise, the identity, or the Σήμα στην εκφώνηση). Server component, zero client JS shipped — L02 bundle stays 158 B / 676 kB (byte-identical to E.3.0 close); /practice +0.02 kB rounding-noise (5.14 → 5.16 kB), /sose unchanged. typecheck+lint+test+build all pass; tests still 20/20. **First per-lecture E.3 retrofit completed.** See [[phase-e3-L02-pairs]])
    - ☑ **L09** (done 2026-05-26 — 6 lecture-page `<RelatedPair>` blocks at the 6 multi-entry pattern moments (P9.1 hand-trace-Dijkstra post-`DijkstraAnimator` `<ExamProblem>` chip; P9.2 Dijkstra-fails-with-negatives post-`DijkstraInvariantBreak` chip; P9.4 ισοβαθμίες ⇒ πολλαπλά ΕΕΔ post-Kruskal-RecallCard before «## Μια τρίτη παραλλαγή»; P9.3 MST-application post-«Prim ή Kruskal;» chip; P9.8 D(MST/TSP/ST) ∈ NP + P9.9 ∈ P για σταθερό k in a NEW `### Η «κατηγοριοποίηση P/NP» γύρω από MST, TSP & Hamilton` mini-section between the «Η αναζήτηση δεν τελείωσε» Callout and «## Κλείδωσε τη γνώση», with a 3-paragraph Greek bridge introducing «πιστοποιητικό + verifier» / «∈ P via optimisation algorithm» / «NP-complete via reduction» recipe) + 22 bidirectional embeds across ALL transcribed L09 entries (pt2-th2-1/pt3-th1/pt1-th1-q6 triangle for P9.1; pt2-th1-q5↔front-set-7-ask10 for P9.2; pt4-th2-b↔front-set-7-ask6 for P9.3; pt4-th2-a↔pt1-th2-b for P9.4; pt5-th3-b/pt7-th4/front-set-10-ask10/pt6-th4/pt16-th5 cluster for P9.8 with each entry pointing to 3 of the other 4; front-set-10-ask7/front-set-10-ask8/pt7-th1/front-set-10-ask9/pt5-th3-a cluster for P9.9 with each entry pointing to 3 of the other 4). P9.5/P9.6/P9.7 solos surfaced ONLY via analogy embeds (front-set-6-ask2 → {pt1-th2-b, front-set-7-ask11} same MST swap-arg theory · front-set-7-ask3 → {pt7-th4, front-set-10-ask10} TSP NP-complete motivation · front-set-7-ask11 → {pt4-th2-a, pt1-th2-b} the part-(iii) unique-MST theorem) — no standalone lecture-page block (mirrors L02 P2.4+P2.5 precedent). Templated cues per pair, localized flavor per entry — every entry's cue OPENS with «Εδώ X» naming the current entry's exam-form surface (e.g. pt2-th2-1: «Εδώ ο πίνακας ζητείται ρητά» · pt3-th1: «Εδώ κατασκευάζεις πρώτα τον γράφο» · pt1-th1-q6: «Εδώ ταυτοποιείς το κριτήριο»). For 5-member pairs P9.8 + P9.9, partial sibling lists (3 of 4+) keep blocks readable; the **headline** sibling (pt6-th4 Σεπτ 2023 20%) is explicitly named in every cluster cue. Server component, zero client JS shipped — L09 bundle stays 158 B / 676 kB byte-identical to L02 close; /practice 5.16 kB / 456 kB unchanged; /sose 8.82 kB / 453 kB unchanged. typecheck+lint+test(20/20)+build all green. **Standing lessons added (4 new)**: (a) lecture-page NEW SECTION + Greek-bridge for theory-only patterns whose concept the prose didn't previously teach (P/NP categorisation is the L09 example — bank entries linked but lecture doesn't formally teach the decision-problem framework); (b) partial sibling lists (3 of 4+) for >2-member pairs to keep blocks readable; (c) explicit «headline» callout in cluster cues names the highest-stakes member in every sibling's cue, training prioritisation; (d) per-entry cue OPENS with «Εδώ X» grounding the templated cue to the current entry. See [[phase-e3-L09-pairs]])
    - ☑ **L03** (done 2026-05-26 — 6 lecture-page `<RelatedPair>` blocks at all 6 multi-entry pattern moments + 20 bidirectional embeds across ALL transcribed L03 entries. Two anchor moments paired with existing `<ExamProblem>` chips (P3.5 after pt1-th1-q5 chip surfacing the «3 παραλλαγές της mergesort-καθρέφτη» pt2-th1-q3+front-set-13-ask3 — both Flame siblings; P3.3 after pt4-th1-q4 chip surfacing the front-set-3-ask4 «εκθετική παγίδα από εκθετικό f» variant). One block landed at the «4 αποδείξεις» natural anchor (P3.6 between «βαρετή» Callout and pt1-th1-q5 chip — listing all 5 induction/substitution-method entries front-set-3-ask8 + front-set-11-ask2 + front-set-4-ask2 + front-set-4-ask3 + front-set-4-ask4 with the «ενίσχυση εικασίας» παγίδα operationalised). **NEW lecture-page section** «Τι ζητάει η εξέταση όταν το Master δεν αρκεί — οι τρεις παραλλαγές» (mirror of L09's standing lesson on theory-only patterns) restructured the existing «Πότε ΔΕΝ εφαρμόζεται» bulleted list into 3 mini-sub-sections each with a Greek bridge teaching the technique + RelatedPair: (a) «Επεκτεταμένη περίπτωση — όταν το f(n) έχει log» (P3.1 — extended Master with k=1 → log^(k+1) n in answer), (b) «Αναδρομές με ρίζα — όταν το όρισμα είναι √n» (P3.2 — substitution n=2^m with diagnostic «a=1 → διπλό log, a≥2 → απλό log»), (c) «Γραμμικές ομογενείς αναδρομές — όταν δεν υπάρχει T(n/b)» (P3.4 — characteristic polynomial with «πολλαπλότητα m ⇒ m γραμμικά ανεξάρτητοι όροι» the «r^n + n·r^n + … + n^(m-1)·r^n» recipe). Bidirectional retrofit: 20 transcribed L03 entries each carry a `<RelatedPair>` at the close of their solution: P3.1 (3 entries — front-set-13-ask3 goes to P3.5 for higher Flame density), P3.2 (4 entries — all pair members), P3.3 (2), P3.4 (3), P3.5 (3 — pt1-th1-q5 + pt2-th1-q3 + front-set-13-ask3), P3.6 (5 — partial sibling lists 3 of 4 per L09 standing lesson). Templated cues per pair, per-entry «Εδώ X» openers grounding to the current entry's surface (e.g. pt1-th1-q4: «Εδώ T(√n)+1, a=1 → case 2 → διπλό log»· pt2-th1-q4: «Εδώ 2T(√n)+1, a=2 → case 1 → απλό log, ο συντελεστής σκότωσε το διπλό log»). Cues operationalise: (a) Master Theorem comparison `d` vs `log_b a` recipe (P3.5), (b) extended Master «log πάνω στο κατώφλι ⇒ +1 log» (P3.1), (c) substitution `n = 2^m` with the «case 1/2 determines simple/double log» discriminator (P3.2), (d) `T(n-c) → εκθετικό` separator (P3.3), (e) characteristic polynomial with multiplicity-m rule (P3.4), (f) induction skeleton + «ενίσχυση εικασίας» (P3.6). Server component, zero client JS shipped — L03 bundle stays **158 B / 676 kB byte-identical** to L09 close· /practice 5.16 kB / 456 kB unchanged· /sose 8.82 kB / 453 kB unchanged. typecheck + lint + test (20/20) + build all pass. **Standing lessons added (2 new for E.3)**: (a) **MULTI-PAIR membership**: when an entry belongs to two pairs (front-set-13-ask3 ∈ P3.1 ∩ P3.5), embed pointing to the pair with HIGHER Flame density (P3.5 here, 2 Flame siblings vs P3.1 zero Flame); the OTHER pair's lecture-page block still lists the entry, just doesn't claim its solution-side embed. (b) **CONTRAST-PAIR diagnostic**: when a pair has 2 entries that differ in ONE parameter and produce DIFFERENT answers (P3.2 pt1-th1-q4 vs pt2-th1-q4 — same √n shape, a=1 vs a=2, double log vs single log), make the cue explicit about the discriminator («ο συντελεστής a καθορίζει αν θα δεις απλό ή διπλό log») — this trains the student to read the parameter, not just recognize the shape. See [[phase-e3-L03-pairs]])
    - ☐ L14 (5) ☐ L15 (4)
    - ☐ L04 (3) ☐ L08 (3)
    - ☐ L01 (1–2) ☐ L11 (1–2) ☐ L12 (1–2) ☐ L13 (1–2) ☐ L16 (1–2) ☐ L17 (1–2)
    - ☐ L05 (1) ☐ L06 (1)
    - L07 — documented-and-skipped (no pairs in E.2)
...
- ☑ F.1.0 — Pre-flight: file↔set mapping confirmation + `sourcePage` field + ExerciseCard/SoseProblemCard link rendering (done 2026-05-25, commit `103b13c` — confirmed file↔bank-set mapping for all 10 frontistirio PDFs (Έφη Μαλέσιου, 2023–24); two non-obvious findings: (a) F3 PDF labelled session «(5)» dated 16/4 while F4 labelled «(4)» dated 9/4 → file index ≠ chronology, (b) F5_6 is one combined deck mapping to bank Σετ #5 ONLY (not #5+#6). `Exercise.sourcePage?: number` added; ExerciseCard + SoseProblemCard now render «Δες το πρωτότυπο PDF (σελ. N)» deep-link via `${sourceFile}#page=${sourcePage}`. Build green, 20/20 tests, lint warnings all pre-existing. See [[phase-f1-0-preflight]])
