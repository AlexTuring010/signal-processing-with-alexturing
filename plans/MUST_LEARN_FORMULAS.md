...
| C | Remaining chapters (Randomness + Modulation Bridge — flag-only sweep) | placement-(a) on all 5 randomness pages + bridge pre-envelope/complex-envelope/bandpass-spectrum | **DONE** — step `mustlearn-passc-randomness-modulation-bridge`; flag-only callouts (no weight, no refs) on randomness/why, random-processes, stationarity, bridge; psd already had callouts for WK+lti-output-psd; bridge F7 (I/Q) previously fixed |
| C | Foundations — LTI/FS-structural + filter-gain-db flag-only sweep (weight-0) | placement-(a) 11 warning callouts: `bibo-stability`, `convolution-definition`, `convolution-properties`, `lti-frequency-response`+`lti-eigenfunction`, `lti-cosine-response` on `foundations/systems`; `fourier-orthogonality`, `fourier-series-synthesis`+`fourier-series-analysis`, `fourier-series-conjugate-symmetry`, `lti-output-fourier-series` on `foundations/fourier-series`; `filter-gain-db` on `foundations/filters`; placement-(b) vacuous (weight-0); placement-(c) by-derivation ✓ | **DONE** — step `mustlearn-passc-foundations-systems-filters`; gate green (typecheck ✓ lint ✓ build ✓) |
| C | Noise — SNR tail (`am-output-snr`·1, `fm-snr-out`·1, `fm-gain-am`·1 placement-(b); `noise-figure`·0 + `fm-noise-output-psd`·0 flag-only) | placement-(b) `sept25-th2-7` memorizationNote extended (3 output-SNR formulas, βάρος 1 each, self-referential; getCitedExercises=1 ✓ each); placement-(a) `noise/sources §10` noise-figure flag-only callout added; `fm/in-noise §3` fm-noise-output-psd flag-only callout added; `noise/snr §4/§5/§6` VERIFIED present (d11a/d11b); fm-snr-ref weight-1 covered by `noise/snr §4` family callout | **DONE** — step `mustlearn-passc-snr-tail`; gate green |
...
| 7 | fm-snr-ref, am-output-snr, fm-snr-out, fm-gain-am | various | **1 each** | **PASS-C DONE** — placement-(b) `sept25-th2-7` memorizationNote extended (am-output-snr/fm-snr-out/fm-gain-am, βάρος 1 each); placement-(a) verified on `noise/snr §4` (fm-snr-ref family) + §5 (am-output-snr) + §6 (fm-snr-out/fm-gain-am); getCitedExercises=1 ✓ each |
...
| 9 | bandpass-noise-r, noise-figure, fm-noise-output-psd, snr, snr-input, G_proc, RC results, B_N | various | **0** | `noise-figure` flag-only callout → `noise/sources §10` **DONE**; `fm-noise-output-psd` flag-only callout → `fm/in-noise §3` **DONE**; `snr`/`snr-input`/`G_proc` covered by `noise/snr §4` family callout (d11a) **DONE**; remaining (bandpass-noise-r, RC, B_N) — LOWER priority, no further action |
...
| 6 | All FS structural formulas (synthesis, analysis-general, orthogonality, conjugate-sym., bₖ=H·aₖ) | various | **0** | **PASS-C DONE** — flag-only callouts on `foundations/fourier-series`: orthogonality after slide-27 note, synthesis+analysis after key callout, conjugate-symmetry after slide-4 callout, lti-output-fs after key callout |
| 6 | All LTI systems formulas (convolution-def, freq-response, eigenfunction, BIBO) | various | **0** | **PASS-C DONE** — flag-only callouts on `foundations/systems`: bibo after §properties, convolution-def after slide-4 note, convolution-properties after §properties section, freq-response+eigenfunction after slide-18 callout, cosine-response after boxed result |
| 6 | dB gain: 20·log₁₀\|H(f)\| | `filter-gain-db` (formulas.tsx L724, weight 0) | **0** | **PASS-C DONE** — flag-only callout on `foundations/filters` §6α after the formula |
...
**PASS-C DONE (step `mustlearn-passc-randomness-modulation-bridge`)** — flag-only `<Callout type="warning">` added to `randomness/why` §5 (vocabulary table) and `randomness/random-processes` §13 (after summary table). All weight-0 formulas (`random-mean`, `random-autocorr`, `random-cross`, `wss`, ergodicity, random-phase-cosine result) now carry the standard must-learn flag. The two lecture problems (`lec-rp-1`, `lec-rp-2`) are weight-0 — placement-(b) vacuous; no exercises.tsx changes.
...
**PASS-C DONE (step `mustlearn-passc-randomness-modulation-bridge`)** — flag-only `<Callout type="warning">` added for pre-envelope (`x_p = x + jx̂`), complex envelope (`g(t)` + `x = Re{g·e^{j2πfct}}`), and bandpass spectrum (`X(f) = ½[G(f-fc)+G*(-f-fc)]`). F7 (I/Q canonical inline flag) was already fixed in a prior step (line 275 shows "⚠️ Πρέπει να θυμάσαι"). All four non-Hilbert bridge formulas now flagged off-sheet.
...
- **Pass B — RANDOMNESS/WHY weighting DONE** (`mustlearn-passb-foundations-randomness-formulas`) — results in §4B. All formulas weight 0 from the past-exam bank (only lecture exercises exist for this chapter). **Pass C DONE** — flag-only `<Callout type="warning">` added to `randomness/why` §5 covering `random-mean`, `random-autocorr`, `random-cross`, `wss`, ergodicity.
- **Pass B — MODULATION BRIDGE weighting DONE** (`mustlearn-passb-foundations-randomness-formulas`) — results in §5B. All bridge-specific formulas weight 0 (pre-envelope, complex envelope, bandpass spectrum, I/Q canonical not standalone-tested). **Pass C DONE** — flag-only callouts added for pre-envelope, complex envelope, bandpass spectrum; F7 (I/Q) was already fixed; Hilbert correctly on-sheet.
- **Pass B — RANDOMNESS/PSD + REMAINING RANDOMNESS weighting DONE** (`mustlearn-passb-foundations-randomness-formulas`) — results in §7B. `wiener-khinchin` + `lti-output-psd` cross-reference §2B (weight 3 each, no additional exercises). **Pass C (weight-bearing) DONE** — `wiener-khinchin` and `lti-output-psd` already had proper weighted callouts; `wss` / `wss-rx-properties` / `ergodicity` flag-only callouts added to `randomness/stationarity` §3/§5/§11; `random-mean`/`random-autocorr`/`random-cross`/`random-phase-cosine` flag-only callout added to `randomness/random-processes` §13; F13 exponential pair already inline-flagged on psd page.
