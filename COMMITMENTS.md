...
- [x] **Ideal filters and the non-causality trade-off** (closed 2026-05-05)
       ...
       Notes: Section 6 names the four ideal filter types (LP, HP, BP, BS) by their $|H(f)|$ shape, ties them to the even-h(t)/real-H(f) symmetry rule from FT §8 (so the "concrete example" promised by the systems chapter finally lands), and the `<FilterTypeViewer />` viz shows which tones survive each filter type. The §4 `<IdealSincResponseViz />` viz makes the non-causality concrete by drawing the ideal LP's sinc impulse response extending to ±∞ (with the t<0 half shaded) — a 🎯 closes-commitment callout makes the connection explicit. The real-filter spec ($\delta_p, \delta_s, f_p, f_s$) is introduced in §5–§6, and the ideal-vs-real trade-off (sharper cutoff = longer impulse response = more delay) is shown by the `<IdealVsRealFilterViz />` side-by-side comparison in the §7 trade-offs section — placed there, not in §4, so no labelled quantity (ripple, transition band) appears before it is defined.
