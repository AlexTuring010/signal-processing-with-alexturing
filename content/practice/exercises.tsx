  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="«∈ NP, και ∈ P για σταθερό k» — Clique/IS/Path/Hamilton με ωμή βία O(n^k · k²)"
          pairs={[{ id: 'front-set-10-ask8' }, { id: 'pt7-th1' }, { id: 'pt5-th3-a' }]}
          cue={
            <>
              <strong>CLIQUE εδώ· INDEP δίπλα</strong> — το ίδιο σχήμα ωμής βίας <InlineMath>{'O(\\binom{n}{k}\\cdot k^2) = O(n^k\\cdot k^2)'}</InlineMath> στις 4 παραλλαγές: Clique (όλες οι <InlineMath>{'\\binom{k}{2}'}</InlineMath> ακμές υπάρχουν), Independent Set (<code>front-set-10-ask8</code> ή <code>pt7-th1</code> — καμία ακμή δεν υπάρχει), Path (διαδοχικές ακμές υπάρχουν), Hamilton (<code>pt5-th3-a</code> — επιπλέον «όλες οι κορυφές μία φορά»). Δύο βήματα: (i) ∈ NP με πιστοποιητικό = σύνολο <InlineMath>{'k'}</InlineMath> κορυφών + verifier <InlineMath>{'O(k^2)'}</InlineMath> · (ii) ∈ P για σταθερό <InlineMath>{'k'}</InlineMath> με ωμή βία πάνω σε όλα τα υποσύνολα. Σήμα στην εκφώνηση: «σταθερό <InlineMath>{'k = 100'}</InlineMath> / <InlineMath>{'k = 1000'}</InlineMath>», «δείξε ότι ανήκει στο P». Παγίδα: «γενικό <InlineMath>{'k'}</InlineMath>» ⇒ NP-complete, όχι ∈ P. INDEP-Clique είναι <strong>καθρέφτη</strong> (συμπληρωματικός γράφος <InlineMath>{"G'"}</InlineMath>) — η ίδια απόδειξη δουλεύει με μηδαμινή προσαρμογή.
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="«∈ NP, και ∈ P για σταθερό k» — Clique/IS/Path/Hamilton με ωμή βία O(n^k · k²)"
          pairs={[{ id: 'front-set-10-ask7' }, { id: 'pt7-th1' }, { id: 'front-set-10-ask9' }]}
          cue={
            <>
              <strong>INDEP εδώ καθαρό</strong>· σε εξεταστικό (<code>pt7-th1</code> · Ιουν 2022 · 20%) ξανάρχεται με <InlineMath>{'k = 1000'}</InlineMath>· σε CLIQUE (<code>front-set-10-ask7</code>) η ίδια απόδειξη μέσω <strong>καθρέφτη</strong> (συμπληρωματικός γράφος). Συνταγή ίδια: ∈ NP με πιστοποιητικό = σύνολο <InlineMath>{'k'}</InlineMath> κορυφών + verifier <InlineMath>{'O(k^2)'}</InlineMath>· ∈ P για σταθερό <InlineMath>{'k'}</InlineMath> με ωμή βία <InlineMath>{'O(n^k\\cdot k^2)'}</InlineMath>. Για D(Path) (<code>front-set-10-ask9</code>) η ωμή βία δεν χρειάζεται — υπάρχει πολυωνυμικός αλγόριθμος (Bellman-Ford), αλλά η ένταξη ∈ NP χτίζεται με ίδιο μοτίβο πιστοποιητικού. Σήμα στην εκφώνηση: «σταθερό <InlineMath>{'k'}</InlineMath>», «δείξε ότι ∈ P για συγκεκριμένη τιμή». Παγίδα: γενικό <InlineMath>{'k'}</InlineMath> ⇒ NP-complete.
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="Πιστοποιητικό + verifier για ∈ NP — κάθε «υπάρχει X με …;» ίδια συνταγή"
          pairs={[{ id: 'front-set-10-ask7' }, { id: 'pt7-th1' }, { id: 'pt5-th3-a' }]}
          cue={
            <>
              <strong>D(Path) εδώ ∈ P</strong> (Bellman-Ford πολυωνυμικά)· <strong>D(K) σακίδιο</strong> ∈ NP μόνο (NP-complete). Σε άλλα παπερ, η ίδια <em>πιστοποιητικό + verifier</em> συνταγή κλιμακώνεται σε: Clique (<code>front-set-10-ask7</code>), Independent Set (<code>pt7-th1</code> · Ιουν 2022 · 20%), Hamilton Path (<code>pt5-th3-a</code> · Ιουν 2023). Σε όλα τα προβλήματα «υπάρχει X που ικανοποιεί …;»: όρισε ως πιστοποιητικό το X (μονοπάτι/σύνολο/ακολουθία), περίγραψε <em>συγκεκριμένους</em> ελέγχους που πρέπει να περάσει, και δείξε ότι κάθε έλεγχος είναι πολυωνυμικός. Παγίδα: μη συγχέεις «∈ NP» με «λύνεται γρήγορα» — το D(K) ∈ NP αλλά πιθανότατα ΟΧΙ ∈ P. <em>Πάντα</em> το αντικείμενο είναι το πιστοποιητικό· ο αλγόριθμος εύρεσης δεν εμπλέκεται στην απόδειξη ∈ NP.
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="D(MST) ∈ P, D(TSP) NP-complete — η ίδια συνταγή σε 5 παπερ"
          pairs={[{ id: 'pt6-th4' }, { id: 'pt7-th4' }, { id: 'pt5-th3-b' }]}
          cue={
            <>
              <strong>Εδώ MST + TSP ως δίδυμο, με ζήτηση D(MST) ∈ P</strong>· ίδια μηχανική παίζεται και σε εξεταστικά: (i) <code>pt6-th4</code> (Σεπτ 2023 · 20% · <strong>headline</strong>) δίνει το ίδιο D(MST) ντυμένο ως «δέντρο που περιέχει το <InlineMath>{'S'}</InlineMath>», με κρίσιμη παρατήρηση <em>«<InlineMath>{'|S| = n'}</InlineMath> ⇒ MST· γενικό <InlineMath>{'S'}</InlineMath> ⇒ Steiner ⇒ NP-complete»</em>· (ii) <code>pt7-th4</code> (Ιουν 2022) ζητά την κατάταξη και των δύο (MST σε P, TSP NP-complete)· (iii) <code>pt5-th3-b</code> (Ιουν 2023) επικεντρώνεται μόνο στο MST σε 3 σκέλη. Συνταγή κοινή: κατώφλι <InlineMath>{'k'}</InlineMath>, πιστοποιητικό = το αντικείμενο (δέντρο/κύκλος), verifier σε <InlineMath>{'O(n+m)'}</InlineMath>, Kruskal/Prim για ∈ P του MST.
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="Εκτέλεσε Dijkstra βήμα-βήμα — πίνακας ανά γύρο"
          pairs={[{ id: 'pt2-th2-1' }, { id: 'pt3-th1' }]}
          cue={
            <>
              <strong>Εδώ <em>ταυτοποιείς το κριτήριο</em></strong> — αλλού η ίδια μηχανή σε βάζει να τη <em>τρέξεις</em>: (i) πλήρης εκτέλεση + πίνακας ανά βήμα σε δοσμένο γράφο (Σεπτ 2025)· (ii) <em>κατασκευή</em> γράφου με συγκεκριμένα χαρακτηριστικά (μη-αρνητικός κύκλος + 5 ακμές + κορυφή-πηγή) και μετά πλήρης εκτέλεση (Ιουν 2024). Σήμα στην εκφώνηση: «εφάρμοσε», «εκτέλεσε», «πίνακας ανά βήμα», «δείξε τα <InlineMath>{'d'}</InlineMath>-values». Σε κάθε εκδοχή, η σωστή στιγμή να ονομάσεις «Dijkstra» είναι μόλις δεις «κατευθυνόμενος γράφος + θετικά/μη-αρνητικά βάρη + ζητά αποστάσεις από μία αφετηρία». Παγίδα: αν τα βάρη είναι αρνητικά, η σωστή απάντηση είναι Bellman-Ford ([L17](/lectures/L17-dp-iv)), όχι Dijkstra (βλ. <code>pt2-th1-q5</code>).
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="Ισοβαθμίες ⇒ πολλαπλά ΕΕΔ — μέτρα τις πραγματικές επιλογές"
          pairs={[{ id: 'pt4-th2-a' }]}
          cue={
            <>
              <strong>Εδώ <em>μετράς</em> ΕΕΔ σε δοσμένο γράφο</strong> (5 ΕΕΔ, όλα κόστους 24, από <InlineMath>{'\\binom{4}{2}=6'}</InlineMath> ζεύγη ίσων ακμών μείον 1 άκυρο)· στο Σεπτ 2024 Θ.2α (Flame) η ίδια θεωρία έρχεται ανάποδα: <em>κατασκεύασε</em> βάρη ώστε το ΕΕΔ να μην είναι μοναδικό — η σωστή κατασκευή είναι «κύκλος ίδιων βαρών», συνήθως τρίγωνο. Σε κάθε εκδοχή, η συνταγή είναι «<strong>υποχρεωτικά πρώτα, μετά οι ισοβαθμίες</strong>»: τρέξε Kruskal, εντόπισε τι μπαίνει μονοσήμαντα, μέτρα τις πραγματικές επιλογές στα ίσα-βάρη συστατικά. Σήμα στην εκφώνηση: «πλήθος ΕΕΔ», «μοναδικό;», «κατασκεύασε βάρη», «πολλαπλές βέλτιστες». Παγίδα: «<em>διακριτά βάρη ⇒ μοναδικό ΕΕΔ</em>» ναι· αντίστροφο γενικά όχι.
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="Αρνητικά βάρη ⇒ Dijkstra σπάει· η «πρόσθεση σταθεράς» ΔΕΝ σώζει"
          pairs={[{ id: 'front-set-7-ask10' }]}
          cue={
            <>
              <strong>Εδώ ταυτοποιείς «ποιος αλγόριθμος σπάει»</strong> — το φροντιστηριακό σε βάζει στην επόμενη φυσική σκέψη: «πρόσθεσε <InlineMath>{'+c'}</InlineMath> σε κάθε ακμή ώστε όλα να γίνουν θετικά, μετά Dijkstra». Η απάντηση είναι <strong>ΟΧΙ, λάθος</strong>: μονοπάτι με <InlineMath>{'\\ell'}</InlineMath> ακμές χρεώνεται <InlineMath>{'c\\cdot\\ell'}</InlineMath> επιπλέον (αύξηση <em>γραμμική στο μήκος</em>, ασύμμετρη ανά μονοπάτι), οπότε αλλάζει ποιο μονοπάτι είναι το συντομότερο. Παράδειγμα: 3 κορυφές, βάρη <InlineMath>{'-1, -3, -3'}</InlineMath>, <InlineMath>{'c = 4'}</InlineMath> → η σωστή 2-ακμή <InlineMath>{'u\\to v\\to w = -4'}</InlineMath> γίνεται «4» (πληρώνει 2·c = 8 extra), ενώ η απευθείας «1» (πληρώνει 1·c = 4 extra) — ο Dijkstra επιστρέφει την απευθείας, λάθος. Σωστή λύση: <strong>Bellman-Ford</strong> (αν δεν υπάρχουν αρνητικοί κύκλοι) σε <InlineMath>{'O(|V|\\cdot|E|)'}</InlineMath>. <em>Mόνο</em> πολλαπλασιαστική μεταφορά (αξιοπιστίες με <strong>log</strong>) σώζει — βλ. <code>front-set-5-ask6</code> στο [L08](/lectures/L08-graphs-iii).
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="Εκτέλεσε Dijkstra βήμα-βήμα — πίνακας ανά γύρο"
          pairs={[{ id: 'pt3-th1' }, { id: 'pt1-th1-q6' }]}
          cue={
            <>
              <strong>Εδώ ο πίνακας <em>ζητείται ρητά</em></strong> — αλλού η ίδια μηχανή έρχεται με δύο άλλες γωνίες: (i) <em>κατασκεύασε</em> πρώτα έναν γράφο με συγκεκριμένα χαρακτηριστικά και μετά εκτέλεσε (Ιουν 2024 — εκεί η παγίδα είναι «μη-αρνητικός <strong>κύκλος</strong>» ≠ «αρνητική <strong>ακμή</strong>»)· (ii) απλά ταυτοποίησε το <em>άπληστο κριτήριο</em> σε πολλαπλή επιλογή (Ιουν 2025 — η σωστή είναι «μικρότερη τρέχουσα <InlineMath>{'d'}</InlineMath> από <InlineMath>{'s'}</InlineMath>», όχι «ελάχιστη ακμή σε τομή» που είναι Prim). Συνταγή κοινή: <em>extractMin</em> την κορυφή με τη μικρότερη τρέχουσα <InlineMath>{'d'}</InlineMath>, χαλάρωσε όλες τις εξερχόμενες, μην ξαναπιάσεις οριστικοποιημένη. Παγίδα-σήμα: αν το κλειδί δοθεί σαν «κόστος ΜΙΑΣ ακμής προς το δέντρο», ψάχνεις τον <strong>Prim</strong>, όχι τον Dijkstra.
            </>
          }
        />
      </>
  ...
  // ── Ιούνιος 2024 — μεταγραμμένο & χωρισμένο ανά διάλεξη ──────────────
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="Εκτέλεσε Dijkstra βήμα-βήμα — πίνακας ανά γύρο"
          pairs={[{ id: 'pt2-th2-1' }, { id: 'pt1-th1-q6' }]}
          cue={
            <>
              <strong>Εδώ <em>κατασκευάζεις πρώτα</em> τον γράφο και μετά εκτελείς</strong> — αλλού η ίδια μηχανή έρχεται «έτοιμη»: (i) δοσμένος γράφος + <em>πίνακας ανά βήμα</em> ως απαιτούμενη έξοδος (Σεπτ 2025)· (ii) πολλαπλή επιλογή για το <em>άπληστο κριτήριο</em> του Dijkstra (Ιουν 2025 — «μικρότερη τρέχουσα <InlineMath>{'d'}</InlineMath> από <InlineMath>{'s'}</InlineMath>», όχι «ελάχιστη ακμή σε τομή» που είναι Prim). Συνταγή κοινή: <em>extractMin</em> με την μικρότερη τρέχουσα <InlineMath>{'d'}</InlineMath>, χαλάρωσε όλες τις εξερχόμενες, μην ξαναπιάσεις οριστικοποιημένη. Παγίδα-σήμα ειδική στο παρόν θέμα: η εκφώνηση γράφει «μη-αρνητικός <em>κύκλος</em>», όχι «μη-αρνητική <em>ακμή</em>» — ο Dijkstra θέλει το <strong>δεύτερο</strong>, και το πρώτο δεν προσθέτει περιορισμό όταν όλες οι ακμές είναι ήδη <InlineMath>{'\\ge 0'}</InlineMath>.
            </>
          }
        />
      </>
    ...
    solution: (
      <>
        ...
        <RelatedPair
          ...
          cue={
            <>
              <strong>Εδώ <em>κατασκευάζεις</em> ισοβαθμίες</strong> (τρίγωνο A-B-C βάρους 1 → 3 ΕΕΔ από <InlineMath>{'\\binom{3}{2}'}</InlineMath>)· στο Ιουν 2025 Θ.2.2 <em>μετράς</em> ΕΕΔ σε δοσμένο γράφο (5 ΕΕΔ, όλα κόστους 24). Σε κάθε εκδοχή, ίδια συνταγή: (1) τρέξε Kruskal και βρες υποχρεωτικές ακμές (μοναδικά ελαφρύτερες ή γέφυρες χωρίς εναλλακτική) · (2) εντόπισε «νησίδες» που μένουν · (3) μέτρα έγκυρα ζευγάρια ίδιου βάρους που τις ενώνουν, αποκλείοντας όσα αφήνουν νησίδα αποκομμένη. Σήμα στην εκφώνηση: «πλήθος ΕΕΔ», «μοναδικό;», «δώσε βάρη ώστε», «πολλαπλές βέλτιστες λύσεις». Παγίδα: «<strong>διακριτά</strong> βάρη ⇔ μοναδικό ΕΕΔ»· το αντίστροφο όχι αναγκαία — η σωστή ισοδυναμία απαιτεί «<em>κύκλος</em> ίδιων βαρών στο ΕΕΔ», όχι «οποιαδήποτε ισοβαθμία στον γράφο» (βλ. και <code>front-set-7-ask11</code> (iii)).
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="«Συνέδεσε όλα τα Χ με ελάχιστο συνολικό κόστος» ⇒ ΕΕΔ (Prim ή Kruskal)"
          pairs={[{ id: 'front-set-7-ask6' }]}
          cue={
            <>
              <strong>Εδώ τα «δίκτυα δρόμων + χιόνι»</strong>· αλλού τα «τηλεφωνικά δίκτυα + αναβάθμιση». Ίδιο πρόβλημα κάτω από δύο σκηνικά: αναγνώριση «<em>όλα συνδεδεμένα + ελάχιστο κόστος</em>» = ΕΕΔ· εκτέλεση με Prim (PQ από μία κορυφή) ή Kruskal (sort + Union-Find) σε <InlineMath>{'O(m\\log n)'}</InlineMath>· <strong>δήλωση</strong> του αλγορίθμου ρητά πριν την εκτέλεση. Σήμα στην εκφώνηση: «οδικό/τηλεφωνικό/υδρευτικό δίκτυο», «καλωδίωση», «<em>ακριβώς μία διαδρομή</em> ανά ζεύγος» (= δέντρο), «<em>ελάχιστο</em> συνολικό κόστος/μήκος», «αναβάθμιση δικτύου». Παγίδα: «συντομότερη διαδρομή από A σε B» ⇒ Dijkstra, όχι ΕΕΔ (διαφορετικά προβλήματα).
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="Πιστοποιητικό = το αντικείμενο, verifier πολυωνυμικός — οικογένεια NP για γραφικά προβλήματα"
          pairs={[{ id: 'pt7-th1' }, { id: 'front-set-10-ask7' }, { id: 'front-set-10-ask9' }]}
          cue={
            <>
              <strong>Hamilton Path εδώ NP-complete</strong> (μόνο ∈ NP — δεν έχουμε πολυωνυμικό αλγόριθμο)· σε άλλα παπερ η ίδια συνταγή «πιστοποιητικό = το αντικείμενο + verifier» χρησιμοποιείται για: <code>pt7-th1</code> (Ιουν 2022 · 20% — INDEP) όπου <em>επιπλέον</em> δείχνεις ∈ P για σταθερό <InlineMath>{'k'}</InlineMath>· <code>front-set-10-ask7</code> (Clique — ίδιο μοτίβο)· <code>front-set-10-ask9</code> (D(Path) που είναι σε P, και D(K) σακίδιο που είναι μόνο ∈ NP). Σε κάθε πρόβλημα «υπάρχει X που ικανοποιεί …;»: πιστοποιητικό = το X (μονοπάτι/υποσύνολο/ακολουθία), verifier = συγκεκριμένοι έλεγχοι σε πολυωνυμικό χρόνο. Παγίδα-σήμα ειδική στο Hamilton Path: ο verifier ελέγχει <strong>τρία</strong> πράγματα — σωστά άκρα <InlineMath>{'v_1 = s, v_n = t'}</InlineMath>, κάθε κορυφή ακριβώς μία φορά, διαδοχικά ζεύγη είναι ακμές του <InlineMath>{'G'}</InlineMath>. Παράλειψη οποιουδήποτε σπάει την απόδειξη.
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="D(MST) ∈ P, D(TSP) NP-complete — η ίδια συνταγή «πιστοποιητικό + verifier»"
          pairs={[{ id: 'pt6-th4' }, { id: 'pt7-th4' }, { id: 'front-set-10-ask10' }]}
          cue={
            <>
              <strong>Εδώ το «MST απόφασης» καθαρό</strong>· τουλάχιστον 4 άλλα παπερ μεταφέρουν την ίδια συνταγή σε άλλη γωνία: (i) <code>pt6-th4</code> (Σεπτ 2023 · 20%, <strong>headline</strong>) τη ντύνει ως «δέντρο που περιέχει το <InlineMath>{'S'}</InlineMath>», με την παγίδα <em>«<InlineMath>{'|S| = n'}</InlineMath> ⇒ MST· αυθαίρετο <InlineMath>{'S'}</InlineMath> ⇒ Steiner ⇒ NP-complete»</em>· (ii) <code>pt7-th4</code> (Ιουν 2022) πακετάρει <strong>MST vs TSP</strong> ως δίδυμο — μία λέξη («tree» vs «cycle») χωρίζει P από NP-complete· (iii) <code>front-set-10-ask10</code> ζητά την ίδια κατασκευή απόφασης για MST + TSP. Συνταγή κοινή: (1) κατώφλι <InlineMath>{'k'}</InlineMath> · (2) ∈ NP με δέντρο ως πιστοποιητικό + verifier <InlineMath>{'O(n+m)'}</InlineMath> · (3) ∈ P μόνο για MST (Kruskal/Prim <InlineMath>{'O(m\\log n)'}</InlineMath>) — όχι TSP/Hamilton. Σήμα στην εκφώνηση: «πρόβλημα απόφασης», «∈ P/NP/NP-complete», «δέντρο επικάλυψης ≤ k». Παγίδα: «∈ P ⇒ ∈ NP» πάντα ισχύει, αλλά η εκφώνηση τυπικά ζητά <strong>ρητή απόδειξη και των δύο</strong>.
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="D(MST) ∈ P, D(TSP) NP-complete — η ίδια συνταγή σε 5 παπερ"
          pairs={[{ id: 'pt5-th3-b' }, { id: 'front-set-10-ask10' }, { id: 'pt16-th5' }]}
          cue={
            <>
              <strong>Εδώ η <em>headline</em> εκδοχή με την παγίδα «|S|»</strong> — το ίδιο D(MST) ξαναέρχεται καθαρό σε άλλα παπερ: <code>pt5-th3-b</code> (Ιουν 2023, 15%) σε 3 σκέλη χωρίς ντύσιμο· <code>front-set-10-ask10</code> ζευγαρωμένο με D(TSP)· <code>pt7-th4</code> (Ιουν 2022) επίσης MST + TSP δίπτυχο· <code>pt16-th5</code> (Ιουν 2016) ζευγαρωμένο με το Longest Path (που είναι NP-complete σαν και το TSP). Συνταγή κοινή: κατώφλι, πιστοποιητικό = το αντικείμενο, verifier πολυωνυμικός, Kruskal/Prim για ∈ P του MST. <em>Η τοπική παγίδα του παρόντος θέματος</em> — «<strong>|S| = n ⇒ MST</strong> ενώ αυθαίρετο S ⇒ Steiner ⇒ NP-complete» — δεν επιστρέφει αλλού· είναι το «σήμα κατατεθέν» αυτής της εκδοχής.
            </>
          }
        />
      </>
  ...
  // ── Ιούνιος 2022 — μεταγραμμένο & χωρισμένο ανά διάλεξη ──────────────
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="«∈ NP, και ∈ P για σταθερό k» — Clique/IS/Path/Hamilton με ωμή βία O(n^k · k²)"
          pairs={[{ id: 'front-set-10-ask8' }, { id: 'front-set-10-ask7' }, { id: 'pt5-th3-a' }]}
          cue={
            <>
              <strong>Εδώ INDEP σε εξεταστική μορφή</strong> (20%)· σε φροντιστηριακό (<code>front-set-10-ask8</code>) η ίδια απόδειξη για INDEP — και ταυτόσημη συνταγή κλιμακώνεται και στο CLIQUE (<code>front-set-10-ask7</code> · καθρέφτης μέσω συμπληρωματικού γράφου <InlineMath>{"G'"}</InlineMath>). Το <code>pt5-th3-a</code> (Hamilton Path) έχει μόνο το ∈ NP μισό (το ∈ P-για-σταθερό-k δεν έχει νόημα στο Hamilton γιατί ζητάει <strong>όλες</strong> τις κορυφές, οπότε <InlineMath>{'k = n'}</InlineMath> — όχι σταθερά). Συνταγή κοινή: (i) πιστοποιητικό = <InlineMath>{'k'}</InlineMath> κορυφές + verifier <InlineMath>{'O(k^2)'}</InlineMath> · (ii) ωμή βία <InlineMath>{'O(n^k\\cdot k^2)'}</InlineMath> — σταθερός εκθέτης ⇒ πολυωνυμικό. Παγίδα: «<em>πρακτικά γρήγορο</em>» ≠ «<em>πολυωνυμικό</em>» — <InlineMath>{'n^{1000}'}</InlineMath> είναι αχρηστευτικά αργό σε πραγματικό υπολογιστή, αλλά αλγοριθμικά «πολυωνυμικό σταθερού βαθμού».
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="D(MST) vs D(TSP) — «δέντρο εύκολο, κύκλος δύσκολο» σε 4+ παπερ"
          pairs={[{ id: 'pt5-th3-b' }, { id: 'front-set-10-ask10' }, { id: 'pt16-th5' }]}
          cue={
            <>
              <strong>Εδώ το «MST vs TSP» ως καθαρό δίδυμο</strong>· σε άλλα παπερ έρχεται με ντύσιμο: <code>pt5-th3-b</code> (Ιουν 2023) ζητά μόνο το D(MST) σε 3 σκέλη· <code>front-set-10-ask10</code> ζητά D(MST) + D(TSP) με ταυτόσημη μηχανική· <code>pt16-th5</code> (Ιουν 2016) αντικαθιστά το TSP με το <em>Longest Path</em> — εξίσου NP-complete, ίδιο δίδαγμα «<em>μέγιστο</em> δύσκολο, <em>ελάχιστο</em> εύκολο». Συνταγή ίδια: κατώφλι <InlineMath>{'k'}</InlineMath>, πιστοποιητικό + verifier για ∈ NP, αλγόριθμος βελτιστοποίησης για ∈ P (μόνο MST). Σήμα-σήμα-σήμα: «δέντρο» vs «κύκλος Hamilton» / «συντομότερο» vs «μακρύτερο» / «P vs NP-complete» — μια λέξη χωρίζει τις δύο ζώνες.
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          relationship="analogy"
          patternName="Σ/Λ θεωρία MST — μοναδικότητα, max-ακμή, swap-argument"
          pairs={[{ id: 'pt1-th2-b' }, { id: 'front-set-7-ask11' }]}
          cue={
            <>
              <strong>«Η <InlineMath>{'i'}</InlineMath>-στη ελαφρύτερη ακμή ανήκει στο ΕΕΔ;»</strong> ζει στην ίδια οικογένεια με «<em>διακριτά βάρη ⇒ μοναδικό ΕΕΔ;</em>» (Σ/Λ — <code>front-set-7-ask11</code> iii) και «<em>πόσα ΕΕΔ;</em>» σε γράφο με ισοβαθμίες (<code>pt1-th2-b</code>) — όλα ζητούν την ίδια θεωρία αποκοπής + κύκλου. Διπλό test: (α) <em>μετράς τις ήδη τοποθετημένες ακμές</em> πριν την υπό κρίση — αν είναι &lt; 2, δεν κλείνει κύκλος, η ακμή μπαίνει σίγουρα· (β) αν είναι ≥ 2, η ιδιότητα κύκλου μπορεί να τη βγάλει — δες αν είναι η ΜΕΓΙΣΤΗ κάποιου κύκλου (τότε ναι, βγαίνει). Παγίδα-σήμα: η «μέγιστη ακμή του <em>γράφου</em>» ΔΕΝ είναι αναγκαστικά η μέγιστη κανενός κύκλου — μπορεί να είναι γέφυρα (βλ. <code>front-set-7-ask11</code> (i) με <code>MaxEdgeAsBridge</code>). Η «μαγική γραμμή» <InlineMath>{'i = 3'}</InlineMath> είναι το μικρότερο σημείο όπου μπορεί να αρχίσει να αποτυγχάνει η «2η/3η/…»-γενίκευση.
            </>
          }
        />
      </>
...
K = 101     Σ = 100`}</pre>
  ...
  4. επίστρεψε τον κύκλο Hamilton που επισκέπτεται
     τις κορυφές με τη σειρά L (και επιστρέφει στο r)`}</pre>
        ...
        <RelatedPair
          relationship="analogy"
          patternName="Γιατί χρειαζόμαστε προσέγγιση — D(TSP) είναι NP-complete"
          pairs={[{ id: 'pt7-th4' }, { id: 'front-set-10-ask10' }]}
          cue={
            <>
              <strong>«Δώσε εφικτή λύση TSP»</strong> έχει νόημα μόνο επειδή η <em>βέλτιστη</em> δεν είναι εύκολη: το <strong>D(TSP)</strong> ανήκει στην <strong>NP</strong> (πιστοποιητικό = ο ίδιος ο κύκλος, verifier ελέγχει «κάθε κορυφή μία φορά» + «βάρος <InlineMath>{'\\le k'}</InlineMath>») αλλά είναι <strong>NP-complete</strong> — δεν ξέρουμε πολυωνυμικό αλγόριθμο. Σε αντίθεση, το D(MST) ανήκει στην <strong>P</strong> (Kruskal/Prim πολυωνυμικά). Αυτή ακριβώς η ασυμμετρία είναι ο λόγος που η προσέγγιση «MST preorder» έχει νόημα: <em>δανείζομαι την ευκολία του MST για να φτιάξω σκελετό για το δύσκολο TSP</em>. Παγίδα-σήμα: αν η εκφώνηση πει «<em>μετρικός</em> TSP» (τριγωνική ανισότητα), τότε το preorder δίνει 2-προσέγγιση· χωρίς τριγωνική ανισότητα, μπορεί να γίνει αυθαίρετα κακή.
            </>
          }
        />
      </>
    ...
    solution: (
      <>
        ...
        <RelatedPair
          ...
          cue={
            <>
              <strong>Εδώ τηλεφωνικό δίκτυο</strong>· στις εξετάσεις (Σεπτ 2024 Θ.2β · Flame) το ίδιο πρόβλημα έρχεται με «<em>οδικό</em> δίκτυο επαρχιακών πόλεων + χειμώνας» + ρητή απαίτηση «αναφέροντας ποιον αλγόριθμο χρησιμοποιείς». Συνταγή κοινή: αναγνώρισε «<em>όλα συνδεδεμένα + ελάχιστο κόστος</em>» = ΕΕΔ, διάλεξε Prim ή Kruskal, εκτέλεσε βήμα-βήμα δηλώνοντας ρητά τον αλγόριθμο. Στην εκφώνηση 2024 υπάρχει και ένα <em>πρώτο</em> υποερώτημα (α) — δώσε βάρη ώστε το ΕΕΔ να μην είναι μοναδικό· εκεί παίζει η ιδιότητα κύκλου σε «κύκλο ίδιων βαρών» (βλ. <code>pt4-th2-a</code> / <code>pt1-th2-b</code>).
            </>
          }
        />
      </>
    ...
    solution: (
      <>
        ...
        <RelatedPair
          ...
          cue={
            <>
              <strong>Εδώ η εξέταση σε προκαλεί με τη «φυσική» διόρθωση «+ σταθερά»</strong> και σε κάνει να αποδείξεις γιατί σπάει· σε άλλη εκδοχή (Σεπτ 2025) απλώς σου ζητάει να <em>αναγνωρίσεις</em> «ποιος αλγόριθμος σπάει με αρνητικά;» μέσα σε λίστα Prim/BFS/Dijkstra/Bellman-Ford. Σε κάθε γωνία, το διάγνωσιμό είναι το ίδιο: ο <strong>Dijkstra εμπιστεύεται οριστικοποίηση</strong> — μόλις μια κορυφή κλειδώσει, δεν την ξανακοιτάζει — οπότε μια αρνητική ακμή που εμφανίζεται αργότερα δεν προφταίνει να βελτιώσει την κλειδωμένη απόσταση. Prim (κλειδί = κόστος <em>μίας</em> ακμής σε τομή, όχι συσσωρευτικό), BFS (αγνοεί βάρη), Bellman-Ford (επανεξετάζει κάθε ακμή σε κάθε γύρο) δεν εμπίπτουν σε αυτή την παγίδα. Σήμα στην εκφώνηση: «αρνητικά βάρη», «τροποποιημένος Dijkstra», «πρόσθεσε σταθερά», «<InlineMath>{'+c'}</InlineMath> σε κάθε ακμή».
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          relationship="analogy"
          patternName="Μοναδικότητα ΕΕΔ + θεωρία κύκλου/αποκοπής — οι Σ/Λ ποτέ δεν λείπουν"
          pairs={[{ id: 'pt4-th2-a' }, { id: 'pt1-th2-b' }]}
          cue={
            <>
              <strong>Το ερώτημα (iii) εδώ είναι «<em>διακριτά βάρη ⇒ μοναδικό ΕΕΔ</em>»</strong> — ίδιο θεώρημα που τα <code>pt4-th2-a</code> (Σεπτ 2024 · Flame) και <code>pt1-th2-b</code> (Ιουν 2025 · Flame) χρησιμοποιούν, αλλά ανάποδα: εκεί <em>κατασκευάζεις/μετράς</em> ΕΕΔ σε γράφο με <strong>ισοβαθμίες</strong>, που είναι το αντίθετο του «διακριτά βάρη». Το ερώτημα (i) εδώ (μέγιστη ακμή ⇒ έξω από ΕΕΔ;) έχει επίσης παρόμοιο πρότυπο σκέψης: η ιδιότητα κύκλου λέει «μέγιστη <em>κύκλου</em>», όχι «μέγιστη <em>γράφου</em>» — αν η μέγιστη του γράφου είναι γέφυρα, μπαίνει πάντα. Όλα τα τρία ζητούν την ίδια διπλή δομή θεωρίας: <strong>αποκοπή</strong> ⇒ ποιες ακμές <em>μπαίνουν</em>· <strong>κύκλος</strong> ⇒ ποιες ακμές <em>βγαίνουν</em>· <strong>μοναδικότητα</strong> ⇒ «κανείς κύκλος ίδιων βαρών στο ΕΕΔ».
            </>
          }
        />
      </>
  ...
  },
  {
    ...
    solution: (
      <>
        ...
        <RelatedPair
          patternName="D(MST) ∈ P, εναλλακτικό «δύσκολο» αδέλφι NP-complete — 5 παπερ"
          pairs={[{ id: 'pt5-th3-b' }, { id: 'pt7-th4' }, { id: 'front-set-10-ask10' }]}
          cue={
            <>
              <strong>Εδώ το «δύσκολο αδέλφι» είναι Longest Path</strong> (μεγιστοποίηση μονοπατιού)· σε άλλα παπερ είναι το <strong>TSP</strong>: <code>pt7-th4</code> (Ιουν 2022) πακετάρει MST + TSP, <code>front-set-10-ask10</code> ζητά την ίδια κατασκευή. Το <code>pt5-th3-b</code> (Ιουν 2023) επικεντρώνεται μόνο στο MST. Σε όλες τις εκδοχές το δίδαγμα είναι ίδιο: μια μικρή αλλαγή στη διατύπωση («δέντρο» → «κύκλος»· «συντομότερο» → «μακρύτερο») σε στέλνει από P σε NP-complete. Συνταγή κοινή: κατώφλι, πιστοποιητικό + verifier για ∈ NP, αλγόριθμος βελτιστοποίησης για ∈ P (μόνο MST). Παγίδα: «<em>απλό</em> μονοπάτι» (όπως εδώ το <InlineMath>{'P'}</InlineMath>) αποκλείει επανάληψη κορυφών — γι' αυτό η μεγιστοποίηση είναι δύσκολη (επιστρέφει στο Hamilton Path).
            </>
          }
        />
      </>
