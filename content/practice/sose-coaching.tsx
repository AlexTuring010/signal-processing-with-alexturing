...
export const SOSE_COACHING: Record<string, ExerciseCoaching> = {
  ...
  'jun26-th1-1': {
    takeaway: (
      <p>
        Κάθε διαφορά AM–FM ξεκλειδώνει από μία μόνο ερώτηση:{' '}
        <strong>ποιο χαρακτηριστικό του carrier κουβαλάει το μήνυμα</strong> — το πλάτος ή τη
        συχνότητα. Απάντησε πρώτα αυτό και μετά διάβασε bandwidth, ισχύ, θόρυβο και δέκτη σαν{' '}
        <em>συνέπειες</em>, όχι σαν πέντε ανεξάρτητα γεγονότα προς αποστήθιση: αν η
        πληροφορία είναι στο πλάτος, ο θόρυβος πλάτους μπαίνει μέσα και η φέρουσα πρέπει να
        εκπεμφθεί· αν είναι στη γωνία, ο limiter επιτρέπεται να πετάξει το πλάτος και η ισχύς
        μένει <InlineMath>{'A_c^2/2'}</InlineMath> ό,τι κι αν κάνεις. Το ίδιο εργαλείο
        δουλεύει και για PM, DSB-SC ή SSB.
      </p>
    ),
    examRadar: (
      <p>
        «Να εξηγηθεί η διαφορά», «συγκρίνετε» δύο διαμορφώσεις, <strong>χωρίς κανέναν
        αριθμό στην εκφώνηση</strong> → η άσκηση ζητάει δομή, όχι πράξεις: πίνακα με άξονες,
        ποτέ ελεύθερη παράγραφο. Λέξεις που σε στέλνουν εδώ: «αναλογική AM και FM»,
        «ευαισθησία στον θόρυβο», «απαιτήσεις εύρους ζώνης», «απόδοση ισχύος»,
        «πολυπλοκότητα δέκτη» — αν σου δίνει τους άξονες, απάντησε με τη σειρά που τους
        δίνει. Ξεκίνα πάντα από τις δύο εξισώσεις: απάντηση που απαριθμεί ιδιότητες χωρίς να
        γράψει <InlineMath>{'x_{AM}(t)'}</InlineMath> και{' '}
        <InlineMath>{'x_{FM}(t)'}</InlineMath> χάνει τη βάση της.
      </p>
    ),
  },
  ...
  'jun26-th1-3': {
    takeaway: (
      <p>
        Μια ΦΠΙ (φασματική πυκνότητα ισχύος) — και η ΣΑΣ που της αντιστοιχεί — είναι
        περιγραφή <strong>δεύτερης τάξης</strong>: κλειδώνει ισχύ και συσχέτιση, ποτέ
        το σχήμα της κατανομής. Όταν λοιπόν σου ζητούν <em>κατανομή</em>, η απάντηση
        δεν βγαίνει από τα φασματικά δεδομένα αλλά από το{' '}
        <strong>φυσικό μοντέλο</strong> της πηγής — για τον θερμικό θόρυβο, από το
        θεώρημα κεντρικού ορίου (Central Limit Theorem, CLT) πάνω σε πλήθος
        ανεξάρτητων συνεισφορών, που δίνει Gaussian μηδενικής μέσης τιμής. Και μόνο
        αφού ξέρεις ότι είναι Gaussian δικαιούσαι ν&apos; αναβαθμίσεις το
        «ασυσχέτιστα δείγματα» (αυτό το δίνει το επίπεδο φάσμα) σε{' '}
        <strong>«ανεξάρτητα δείγματα»</strong>.
      </p>
    ),
    examRadar: (
      <p>
        Όταν η εκφώνηση λέει «τι κατανομή» / «ΣΠΠ» / «πυκνότητα πιθανότητας» και
        μιλάει για θόρυβο, η απάντηση είναι σχεδόν πάντα{' '}
        <strong>Gaussian μηδενικής μέσης τιμής</strong> — και οι μονάδες κερδίζονται
        στην αιτιολόγηση με CLT (θεώρημα κεντρικού ορίου), όχι σε αναφορά στο επίπεδο
        φάσμα. Πρόσεξε την <strong>κατεύθυνση</strong>: εδώ ρωτάει για τα{' '}
        <em>δείγματα</em> και η σωστή απάντηση είναι «Gaussian», ενώ όταν μια πρόταση
        ισχυρίζεται ότι «η ΦΠΙ ακολουθεί κατανομή Gauss» είναι η κλασική Σ/Λ παγίδα
        και η απάντηση γίνεται ΛΑΘΟΣ. Η λέξη «λευκός» στην εκφώνηση είναι νύξη για τη
        σχέση <em>μεταξύ</em> των δειγμάτων (δέλτα ΣΑΣ → ασυσχέτιστα → iid επειδή
        Gaussian), όχι για το σχήμα της καμπάνας.
      </p>
    ),
    relatedIds: ['jan26-th1-3', 'pb25-th1-3', 'sept25-th3-10', 'proodos26-6'],
  },
  ...
  'jun26-th1-6': {
    takeaway: (
      <p>
        <strong>
          Το ιδανικό φίλτρο δεν πειράζει το ύψος της PSD — αποφασίζει μόνο ποιο
          κομμάτι του άξονα συχνοτήτων επιβιώνει
        </strong>
        , και από αυτό βγαίνουν δύο πολύ διαφορετικά πράγματα: η <em>ισχύς</em> είναι
        σκέτο «ύψος × συνολικό φάσμα» και αγνοεί το πού κάθεται η ζώνη, ενώ η{' '}
        <em>ΣΑΣ</em> (Συνάρτηση Αυτοσυσχέτισης) κρατάει και το πού. Το πλάτος ενός{' '}
        <em>συνεχόμενου</em> κομματιού φάσματος ορίζει (αντιστρόφως ανάλογα) το χρονικό
        εύρος της περιβάλλουσας, και η μετατόπισή του στα{' '}
        <InlineMath>{'\\pm f_c'}</InlineMath> τη γεμίζει με{' '}
        <InlineMath>{'\\cos(2\\pi f_c \\tau)'}</InlineMath> — το ίδιο θεώρημα
        διαμόρφωσης του AM, διαβασμένο από τη συχνότητα προς τον χρόνο. Γι&apos; αυτό
        ένα χαμηλοπερατό και ένα ζωνοπερατό ίδιου συνολικού φάσματος έχουν{' '}
        <strong>ίδια ισχύ αλλά διαφορετική μνήμη</strong>.
      </p>
    ),
    examRadar: (
      <p>
        «Λευκός θόρυβος» + «ιδανικό φίλτρο» + ζητάει <strong>ΣΑΣ</strong> (Συνάρτηση
        Αυτοσυσχέτισης) και όχι ισχύ → μην ξεκινήσεις ολοκλήρωμα στον χρόνο: γράφε{' '}
        <InlineMath>{'S_Y(f) = |H(f)|^2 S_X(f)'}</InlineMath>, κοίτα τι ορθογώνια
        έμειναν, και κάνε αντίστροφο Fourier — αν είναι <em>δύο</em>, γύρω από{' '}
        <InlineMath>{'\\pm f_c'}</InlineMath>, η απάντηση έχει υποχρεωτικά τη μορφή
        «sinc επί συνημίτονο», και αν δεν σου βγήκε συνημίτονο ξέχασες τη μετατόπιση.
        Η παγίδα είναι το «εύρος ζώνης <InlineMath>W</InlineMath>» χωρίς σχήμα: σε
        ζωνοπερατό μπορεί να σημαίνει είτε το πλάτος της κάθε ζώνης είτε{' '}
        <InlineMath>W</InlineMath> εκατέρωθεν του <InlineMath>{'f_c'}</InlineMath>, και
        ένας παράγοντας 2 κρέμεται από αυτό — γι&apos; αυτό γράφεις τα όρια ρητά{' '}
        (<InlineMath>{'f_c - W/2 \\le |f| \\le f_c + W/2'}</InlineMath>) στην πρώτη
        γραμμή και μετά λύνεις.
      </p>
    ),
  },
  ...
  'jun26-th1-7': {
    takeaway: (
      <p>
        Όταν τα κομμάτια ενός σήματος ζουν σε <strong>διαφορετικές συχνότητες</strong>, δεν
        αλληλεπιδρούν ενεργειακά: οι επιμέρους ισχύες απλώς προστίθενται,{' '}
        <InlineMath>{'P = \\sum_{\\kappa} A_\\kappa^2/2'}</InlineMath>. Το κρίσιμο δεν είναι ο
        τύπος αλλά η προϋπόθεσή του — μόλις δύο όροι μοιραστούν την ίδια συχνότητα, το σταυρωτό
        γινόμενο επιβιώνει και η απλή πρόσθεση καταρρέει. Το ίδιο επιχείρημα ορθογωνιότητας
        είναι που κάνει το FDM να δουλεύει και που εξαφανίζει τους όρους διπλής συχνότητας στους
        υπολογισμούς ισχύος της AM.
      </p>
    ),
    examRadar: (
      <p>
        Άθροισμα από sin/cos συν «υπολογίστε την ισχύ» → γράφεις κατευθείαν{' '}
        <InlineMath>{'\\sum A_\\kappa^2/2'}</InlineMath>, αφού πρώτα ελέγξεις με μια ματιά ότι
        καμία συχνότητα δεν επαναλαμβάνεται. Αν τα πλάτη είναι δεμένα με τον δείκτη
        (<InlineMath>{'\\kappa A'}</InlineMath>, <InlineMath>{'(10-\\kappa)A'}</InlineMath> κ.λπ.),
        το επόμενο βήμα είναι σίγουρα άθροισμα τετραγώνων — έχε πρόχειρο το{' '}
        <InlineMath>{'n(n+1)(2n+1)/6'}</InlineMath>. Και αν η εκφώνηση δίνει μια συχνότητα που
        πουθενά δεν χρησιμοποιείς, μην ψάχνεις λάθος: η ισχύς όντως δεν εξαρτάται από αυτήν.
      </p>
    ),
  },
  ...
  'jun26-th1-8': {
    takeaway: (
      <p>
        Το NBFM/WBFM δεν είναι ιδιότητα του διαμορφωτή — είναι ετυμηγορία πάνω σε έναν
        αριθμό, το <InlineMath>{'\\beta_f'}</InlineMath>, και το{' '}
        <InlineMath>{'\\beta_f'}</InlineMath> χτίζεται πάντα από δύο <em>ανεξάρτητα</em>{' '}
        κομμάτια του message: το πλάτος δίνει το{' '}
        <InlineMath>{'\\Delta f = K_f\\max|m|'}</InlineMath> (πόσο μακριά φεύγει η
        συχνότητα) και η συχνότητα/bandwidth δίνει το <InlineMath>W</InlineMath> (πόσο
        γρήγορα). Ο έλεγχος μονάδων είναι το φθηνότερο δίχτυ ασφαλείας που έχεις: ο{' '}
        <InlineMath>{'K_f'}</InlineMath> είναι Hz/Volt, οπότε μπορεί να πολλαπλασιάσει
        μόνο Volt — ποτέ Hz.
      </p>
    ),
    examRadar: (
      <p>
        Όταν η εκφώνηση σου δίνει το <InlineMath>{'m(t)'}</InlineMath> ως συγκεκριμένο
        cosine μαζί με <InlineMath>{'K_f'}</InlineMath> σε Hz/Volt και ρωτάει «NBFM ή
        WBFM», η πρώτη γραμμή σου είναι{' '}
        <InlineMath>{'\\Delta f = K_f\\max|m|'}</InlineMath> και η δεύτερη{' '}
        <InlineMath>{'\\beta_f = \\Delta f / f_m'}</InlineMath>. Το «και γιατί» δεν
        ικανοποιείται με την ταμπέλα: θέλει τουλάχιστον μία πρόταση συνέπειας —{' '}
        <InlineMath>{'\\beta \\ll 1'}</InlineMath> σημαίνει ότι ο Carson καταρρέει στο{' '}
        <InlineMath>{'2W'}</InlineMath> και το φάσμα γίνεται φέρον + ένα ζεύγος sidebands,
        σαν AM. Η ίδια εκφώνηση συνήθως συνεχίζει με «βρες το bandwidth» ή «σχεδίασε το
        φάσμα» — έχεις ήδη το <InlineMath>{'\\beta'}</InlineMath>, οπότε είναι μία ακόμα
        γραμμή.
      </p>
    ),
  },
  ...
  'jun26-th2-10': {
    takeaway: (
      <p>
        <strong>Το κενό των φερόντων χτίζεται από άκρες, όχι από αποστηθισμένους τύπους:</strong>{' '}
        ελάχιστο <InlineMath>{'\\Delta f'}</InlineMath> = (όσο προεξέχει το κάτω κανάλι δεξιά από το
        φέρον του) + (όσο προεξέχει το πάνω κανάλι αριστερά από το δικό του). Ένα USSB κανάλι από
        πάνω συνεισφέρει <strong>μηδέν</strong> στον δεύτερο όρο — γι&rsquo; αυτό εδώ αρκεί{' '}
        <InlineMath>{'W/2'}</InlineMath>, ενώ με DSB από πάνω θα χρειαζόταν{' '}
        <InlineMath>{'13W/2'}</InlineMath>. Και όταν το ζητούμενο δίνεται ως λόγος{' '}
        <InlineMath>{'f_2 = n f_1'}</InlineMath>, η απάντηση βγαίνει πάντα στη μορφή{' '}
        <InlineMath>{'n \\ge 1 + \\Delta f_{\\min}/f_1'}</InlineMath>: δέσ&rsquo; την με τη συνθήκη
        μη-διπλώματος στο DC (<InlineMath>{'f_1 \\ge B_m'}</InlineMath>) και το{' '}
        <InlineMath>{'n'}</InlineMath> κλειδώνει σε συγκεκριμένο αριθμό.
      </p>
    ),
    examRadar: (
      <p>
        Μόλις δεις «<InlineMath>{'f_2 = n f_1'}</InlineMath>» αντί για ελεύθερο δεύτερο φέρον, ξέρεις
        ότι η απάντηση δεν είναι αριθμός αλλά ανισότητα στο <InlineMath>{'n'}</InlineMath> — και ότι
        για να καταλήξεις σε συγκεκριμένο ακέραιο θα χρειαστείς <em>και</em> τη συνθήκη
        μη-διπλώματος στο DC του κάτω καναλιού. Δεύτερο σήμα κατατεθέν:{' '}
        <strong>δύο διαφορετικά σχήματα διαμόρφωσης στην ίδια εκφώνηση</strong> (εδώ DSB-SC και USSB)
        σημαίνει ότι ο έτοιμος κανόνας <InlineMath>{'\\Delta f \\ge 2W'}</InlineMath> δεν ισχύει:
        σχεδίασε τις δύο λωρίδες και διάβασε τις άκρες τους. Τρίτο, και το πιο ακριβό αν το χάσεις:{' '}
        <InlineMath>{'\\mathrm{sinc}(Wt)'}</InlineMath> έχει μισό-εύρος{' '}
        <InlineMath>{'W/2'}</InlineMath> ενώ <InlineMath>{'\\mathrm{sinc}^2(6Wt)'}</InlineMath> έχει{' '}
        <InlineMath>{'6W'}</InlineMath> — το τετράγωνο <em>πλαταίνει</em> το φάσμα, δεν το στενεύει.
      </p>
    ),
  },
  ...
  'jun26-th2-12': {
    takeaway: (
      <p>
        <strong>
          Ενέργεια πολυπλεγμένου = (ενέργεια κάθε baseband μηνύματος) × (ο συντελεστής της
          διαμόρφωσής του), και μετά σκέτη πρόσθεση — αρκεί τα φάσματα να μην αγγίζονται.
        </strong>{' '}
        Οι συντελεστές είναι <InlineMath>{'1/2'}</InlineMath> για DSB-SC και{' '}
        <InlineMath>{'1'}</InlineMath> για SSB στην πλήρη μορφή: ακριβώς οι ίδιοι που είδες στις
        ισχύες, γιατί προκύπτουν από την ίδια κίνηση (<InlineMath>{'\\cos^2'}</InlineMath> σε
        μισό συν διπλάσια συχνότητα, και ο όρος διπλάσιας συχνότητας μηδενίζεται{' '}
        <em>ακριβώς</em> όταν το φέρον ξεπερνά το bandwidth του μηνύματος). Ό,τι δεν βγαίνει στον
        χρόνο — <InlineMath>{'\\int \\mathrm{sinc}^2'}</InlineMath>,{' '}
        <InlineMath>{'\\int \\mathrm{sinc}^4'}</InlineMath> — το πηγαίνεις στη συχνότητα με
        Parseval, όπου γίνεται εμβαδόν ορθογωνίου ή τριγώνου· και σε κάθε πρόβλημα SSB χωρίς
        δοσμένο <InlineMath>{'A_c'}</InlineMath>, γράψε ποια μορφή χρησιμοποιείς πριν γράψεις
        αριθμό.
      </p>
    ),
    examRadar: (
      <>
        <p>
          Μηνύματα δοσμένα σαν <InlineMath>{'\\mathrm{sinc}'}</InlineMath> ή{' '}
          <InlineMath>{'\\mathrm{sinc}^2'}</InlineMath> + η λέξη{' '}
          <strong>«ενέργεια»</strong> (όχι «ισχύς») → αυτά είναι energy signals με{' '}
          <em>μηδενική</em> μέση ισχύ, οπότε κάθε τύπος με <InlineMath>{'P_m'}</InlineMath> σε
          οδηγεί στο μηδέν. Το πρώτο που γράφεις είναι Parseval και τα δύο ζεύγη του τυπολογίου
          (<InlineMath>{'\\Pi \\leftrightarrow \\mathrm{sinc}'}</InlineMath>,{' '}
          <InlineMath>{'\\Lambda \\leftrightarrow \\mathrm{sinc}^2'}</InlineMath>)· το δεύτερο
          είναι η μορφή του SSB σήματος. Αν το ερώτημα έρχεται αμέσως μετά από ένα «πόσο πρέπει
          να είναι το <InlineMath>{'n'}</InlineMath>», η μη-επικάλυψη σού έχει ήδη δοθεί —
          επικαλέσου την ρητά για να δικαιολογήσεις ότι οι ενέργειες προστίθενται. Είναι 8% και
          καθαρά υπολογιστικό: <strong>χρόνος-στόχος ~10–12 λεπτά</strong>.
        </p>
        <div className="my-2 rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
          ...
          <strong>Δεν δηλώνεις τη μορφή της SSB:</strong> πλήρης{' '}
          <InlineMath>{'A_c[k\\cos - \\hat{k}\\sin]'}</InlineMath> έναντι μισής{' '}
          <InlineMath>{'\\tfrac{A_c}{2}[\\cdot]'}</InlineMath> → διαφορά ×4 στην ενέργεια του
          δεύτερου καναλιού. (2) <strong>Ξεχνάς τον Hilbert όρο</strong> και γράφεις{' '}
          <InlineMath>{'E_2 = A_c^2 E_k/2'}</InlineMath> σαν να ήταν DSB-SC — η SSB δεν έχει τον
          παράγοντα <InlineMath>{'1/2'}</InlineMath>. (3){' '}
          <strong>Βάζεις <InlineMath>{'B_m = W'}</InlineMath>:</strong> το{' '}
          <InlineMath>{'\\mathrm{sinc}(Wt)'}</InlineMath> δίνει rect{' '}
          <em>συνολικού</em> πλάτους <InlineMath>{'W'}</InlineMath>, άρα{' '}
          <InlineMath>{'B_m = W/2'}</InlineMath>. (4){' '}
          <strong>
            Χρησιμοποιείς <InlineMath>{'\\int \\mathrm{sinc}^2 = 1/a'}</InlineMath> για το{' '}
            <InlineMath>{'k'}</InlineMath>:
          </strong>{' '}
          το <InlineMath>{'k = \\mathrm{sinc}^2'}</InlineMath> θέλει{' '}
          <InlineMath>{'\\int \\mathrm{sinc}^4 = 2/(3a)'}</InlineMath> — ο συντελεστής{' '}
          <InlineMath>{'2/3'}</InlineMath> έρχεται από το{' '}
          <InlineMath>{'\\int \\Lambda^2'}</InlineMath>. (5){' '}
          <strong>Ψάχνεις τύπο ισχύος:</strong> εδώ κάθε{' '}
          <InlineMath>{'P'}</InlineMath> είναι μηδέν — η ερώτηση είναι ενεργειακή.
        </div>
      </>
    ),
  },
  ...
  'jun26-th2-13': {
    takeaway: (
      <p>
        Ο ανιχνευτής περιβάλλουσας δεν επιστρέφει «το μήνυμα» — επιστρέφει{' '}
        <InlineMath>{'\\sqrt{x_I^2 + x_Q^2}'}</InlineMath>, οπότε δίνει το{' '}
        <InlineMath>{'x_I'}</InlineMath> μόνο αν ισχύουν <em>ταυτόχρονα</em> δύο πράγματα:{' '}
        <strong>(A)</strong> <InlineMath>{'x_Q \\equiv 0'}</InlineMath> και{' '}
        <strong>(B)</strong> <InlineMath>{'x_I(t) \\ge 0'}</InlineMath> για κάθε{' '}
        <InlineMath>t</InlineMath>. Γράψε λοιπόν κάθε διαμορφωμένο σήμα στη μορφή{' '}
        <InlineMath>{'x_I\\cos - x_Q\\sin'}</InlineMath> και τσέκαρε τα δύο κουτάκια: το
        DSB-SC κόβεται στο (B), το SSB κόβεται στο (A), και μόνο το συμβατικό AM τα περνά και
        τα δύο — επειδή ακριβώς πληρώνει μεταδιδόμενο φέρον με{' '}
        <InlineMath>{'\\mu \\le 1'}</InlineMath>. Το μεταφέρσιμο μάθημα: «το μήνυμα είναι
        θετικό» δεν είναι επαρκής λόγος για να δουλέψει ο ανιχνευτής, γιατί στο SSB το
        πρόβλημα δεν είναι το πρόσημο αλλά ο όρος Hilbert.
      </p>
    ),
    examRadar: (
      <p>
        Μόλις δεις «διατίθεται μόνο ανιχνευτής περιβάλλουσας» ή «φτηνός/ασύμφωνος δέκτης»,
        γύρνα στην εκφώνηση και ψάξε ένα και μόνο πράγμα: <strong>μεταδίδεται φέρον;</strong>{' '}
        Οι λέξεις «SC», «DSB-SC», «SSB», «USSB», «LSB» σημαίνουν όχι ⇒ χρειάζεται{' '}
        <em>σύμφωνη</em> αποδιαμόρφωση· οι λέξεις «συμβατικό AM» ή η μορφή{' '}
        <InlineMath>{'[A_c + m(t)]\\cos'}</InlineMath> σημαίνουν ναι, και τότε το αμέσως
        επόμενο τσεκ είναι <InlineMath>{'\\mu \\le 1'}</InlineMath>. Η ειδική παγίδα αυτού του
        θέματος είναι το μη αρνητικό μήνυμα (<InlineMath>{'\\mathrm{sinc}^2'}</InlineMath>,
        τετράγωνο, ανορθωμένο, «πάντα θετικό»): σε παρασύρει να πεις «άρα ανιχνεύεται», ενώ σε
        SSB η θετικότητα είναι άσχετη — και αντίστροφα, αν το κανάλι είναι DSB-SC κοίτα αν το
        μήνυμα αλλάζει πρόσημο (το <InlineMath>{'\\mathrm{sinc}'}</InlineMath> αλλάζει, στους
        πλευρικούς λοβούς).
      </p>
    ),
  },
  ...
  'jun26-th3-14': {
    takeaway: (
      <p>
        Ο δείκτης <InlineMath>{'\\beta'}</InlineMath> μετράει την απόκλιση συχνότητας{' '}
        <strong>σε μονάδες bandwidth του message</strong>:{' '}
        <InlineMath>{'\\beta = \\Delta f / W'}</InlineMath>, και γίνεται{' '}
        <InlineMath>{'\\Delta f / f_m'}</InlineMath> μόνο επειδή ένας τόνος έχει{' '}
        <InlineMath>{'W = f_m'}</InlineMath>. Η σχέση δεν είναι τύπος προς
        αποστήθιση — βγαίνει σε δύο γραμμές παραγωγίζοντας τη φάση, αφού{' '}
        <InlineMath>{'f_i = f_c + \\beta f_m\\cos(2\\pi f_m t)'}</InlineMath>. Και
        επειδή το <InlineMath>{'\\beta'}</InlineMath> ζει μέσα στη γωνία, το πλάτος
        του φέροντος δεν το επηρεάζει ποτέ: ό,τι <InlineMath>{'A_c'}</InlineMath> κι
        αν σου δώσουν, το <InlineMath>{'\\beta'}</InlineMath> βγαίνει ίδιο.
      </p>
    ),
    examRadar: (
      <p>
        Αν δεις FM μαζί με <InlineMath>{'\\Delta f'}</InlineMath> και{' '}
        <InlineMath>{'f_m'}</InlineMath> — ή με <InlineMath>{'K_f'}</InlineMath> και
        πλάτος message, οπότε πρώτα{' '}
        <InlineMath>{'\\Delta f = K_f\\max|m|'}</InlineMath> — η πρώτη γραμμή που
        γράφεις είναι το <InlineMath>{'\\beta'}</InlineMath>. Σχεδόν ποτέ δεν σταματάει
        εκεί: το <InlineMath>{'\\beta'}</InlineMath> είναι η είσοδος και για τον Carson
        και για τον πίνακα Bessel, οπότε υπολόγισέ το ακόμα κι αν το ερώτημα δεν το
        ζητάει ρητά. Ο δωρεάν έλεγχος δεν είναι ο πίνακας — ο πίνακας έχει αραιές
        γραμμές και ένα σωστό <InlineMath>{'\\beta'}</InlineMath> μπορεί κάλλιστα να
        πέσει ανάμεσά τους. Ο έλεγχος είναι ο πολλαπλασιασμός ανάποδα:{' '}
        <InlineMath>{'\\beta f_m'}</InlineMath> πρέπει να σου ξαναδώσει τη δοσμένη{' '}
        <InlineMath>{'\\Delta f'}</InlineMath>.
      </p>
    ),
  },
  ...
  'jun26-th3-15': {
    takeaway: (
      <p>
        Ο Carson είναι ένας κανόνας με δύο πρόσωπα:{' '}
        <InlineMath>{'2(\\Delta f + W)'}</InlineMath> όταν το θέμα σου δίνει απόκλιση και
        εύρος message, <InlineMath>{'2W(\\beta+1)'}</InlineMath> όταν σου δίνει τον
        δείκτη — και περνάς από το ένα στο άλλο βάζοντας{' '}
        <InlineMath>{'\\Delta f = \\beta W'}</InlineMath>, όχι θυμούμενος δύο τύπους. Το
        bandwidth του FM δεν εξαρτάται ούτε από το <InlineMath>{'A_c'}</InlineMath> ούτε
        από το <InlineMath>{'f_c'}</InlineMath>: είναι πλάτος ζώνης, όχι ισχύς ούτε
        θέση. Οι συντομεύσεις <InlineMath>{'2W'}</InlineMath> (NBFM) και{' '}
        <InlineMath>{'2\\Delta f'}</InlineMath> (WBFM) είναι μόνο για τα άκρα — σε
        ενδιάμεσο <InlineMath>{'\\beta'}</InlineMath> τις πληρώνεις με σφάλμα δεκάδων
        τοις εκατό.
      </p>
    ),
    examRadar: (
      <p>
        Αν στην εκφώνηση δεις μαζί <InlineMath>{'\\Delta f'}</InlineMath> και{' '}
        <InlineMath>{'f_m'}</InlineMath> (ή <InlineMath>{'K_f'}</InlineMath> και πλάτος
        message), η ερώτηση «εύρος ζώνης κατά Carson» έρχεται σχεδόν σίγουρα — γράψε
        πρώτη γραμμή <InlineMath>{'\\beta = \\Delta f / f_m'}</InlineMath> και δεύτερη{' '}
        <InlineMath>{'B = 2(\\Delta f + f_m)'}</InlineMath>. Αν αντίθετα σου δίνουν το{' '}
        <InlineMath>B</InlineMath> και ζητάνε <InlineMath>{'\\beta'}</InlineMath> ή
        πλάτος message, είναι ο ίδιος τύπος ανάποδα:{' '}
        <InlineMath>{'\\beta = B/(2f_m) - 1'}</InlineMath>. Και μόλις το message πάψει να
        είναι ένας τόνος (sinc, άθροισμα αρμονικών), το{' '}
        <InlineMath>W</InlineMath> δεν διαβάζεται από την εκφώνηση — βγάλ&apos; το πρώτα
        από το φάσμα του <InlineMath>{'m(t)'}</InlineMath> και μετά βάλ&apos; το στον
        Carson.
      </p>
    ),
  },
  ...
  'jun26-th3-16': {
    takeaway: (
      <p>
        Ο πίνακας Bessel είναι ευρετηριασμένος στο{' '}
        <InlineMath>{'\\beta'}</InlineMath>, όχι στο{' '}
        <InlineMath>{'\\Delta f'}</InlineMath> — γι&apos; αυτό η πρώτη γραμμή που
        γράφεις σε κάθε τέτοιο θέμα είναι ο δείκτης{' '}
        <InlineMath>{'\\beta = \\Delta f / W'}</InlineMath>, που εδώ γίνεται{' '}
        <InlineMath>{'\\Delta f / f_m'}</InlineMath> μόνο επειδή το message είναι ένας
        τόνος και άρα <InlineMath>{'W = f_m'}</InlineMath>· αλλιώς διαβάζεις λάθος
        σειρά. Μετά το πλάτος κάθε γραμμής είναι απλώς{' '}
        <InlineMath>{'A_c|J_n(\\beta)|'}</InlineMath>, και το πρόσημο του{' '}
        <InlineMath>{'J_n'}</InlineMath> δεν είναι πλάτος αλλά φάση{' '}
        <InlineMath>{'180^\\circ'}</InlineMath>. Το «εντός του ενεργού φάσματος» είναι
        εντολή μετρήματος, όχι διακόσμηση: είναι ο Carson μεταμφιεσμένος και σου λέει
        να σταματήσεις στο{' '}
        <InlineMath>{'|n| \\le \\lfloor\\beta\\rfloor + 1'}</InlineMath> — εδώ 4, γιατί
        το <InlineMath>{'\\beta'}</InlineMath> είναι ακέραιο· για{' '}
        <InlineMath>{'\\beta = 2.5'}</InlineMath> θα ήταν 3, όχι 3.5, αφού το{' '}
        <InlineMath>n</InlineMath> μετράει γραμμές.
      </p>
    ),
    examRadar: (
      <p>
        Αν η εκφώνηση δίνει <InlineMath>{'\\Delta f'}</InlineMath> και{' '}
        <InlineMath>{'f_m'}</InlineMath> και λέει «με χρήση πίνακα Bessel», η κίνηση
        είναι μηχανική: <InlineMath>{'\\beta = \\Delta f/f_m'}</InlineMath> → βρες τη
        γραμμή → πολλαπλασίασε επί <InlineMath>{'A_c'}</InlineMath>. Οι φράσεις «εντός
        του ενεργού φάσματος», «σημαντικές αρμονικές» ή «πόσες γραμμές» σε στέλνουν
        πάντα στον Carson: <InlineMath>{'|n| \\le \\lfloor\\beta\\rfloor + 1'}</InlineMath>,
        σύνολο <InlineMath>{'2\\lfloor\\beta\\rfloor+3'}</InlineMath> γραμμές (το πάτωμα
        μετράει: <InlineMath>{'\\beta = 2.5'}</InlineMath> δίνει{' '}
        <InlineMath>{'|n| \\le 3'}</InlineMath> και 7 γραμμές, όχι 3.5). Αν σου δίνουν{' '}
        <InlineMath>{'A_c'}</InlineMath> σε Volt θέλουν πραγματικά Volt· αν δεν δίνεται
        καθόλου <InlineMath>{'A_c'}</InlineMath>, θέλουν σχετικές εντάσεις — και αν το{' '}
        <InlineMath>{'\\beta'}</InlineMath> πέφτει κοντά σε ρίζα του{' '}
        <InlineMath>{'J_0'}</InlineMath> (2.405, 5.520, 8.654), περίμενε το follow-up
        «πότε εξαφανίζεται ο carrier».
      </p>
    ),
  },
  ...
  'jan26-th1-5': {
    takeaway: (
      <p>
        Το όριο NBFM/WBFM που μετράει στο μάθημα είναι{' '}
        <InlineMath>{'\\beta \\ll 1'}</InlineMath> (NBFM — φάσμα φέρον + ένα
        ζεύγος sidebands, <InlineMath>{'B \\cong 2W'}</InlineMath> όπως στην AM)
        έναντι <InlineMath>{'\\beta \\gg 1'}</InlineMath> (WBFM — πολλές
        sidebands, πολύ μεγαλύτερο bandwidth), <strong>όχι</strong> το «
        <InlineMath>{'\\beta < 0.3'}</InlineMath>» που δίνουν κάποια εγχειρίδια.
        Στο boundary <InlineMath>{'\\beta \\sim 1'}</InlineMath> δεν υπάρχει
        αυστηρή γραμμή.
      </p>
    ),
    examRadar: (
      <p>
        Σ/Λ για το αν συγκεκριμένο <InlineMath>{'\\beta'}</InlineMath> είναι
        NBFM ή WBFM → σύγκρινέ το με τη <strong>μονάδα</strong>, όχι με το 0.3:{' '}
        <InlineMath>{'\\beta \\ll 1'}</InlineMath> NBFM,{' '}
        <InlineMath>{'\\beta \\gg 1'}</InlineMath> WBFM. Οι παγίδες δίνουν
        0.3, 0.5 ή 1 και ζητάνε ταξινόμηση — και το 0.3 είναι{' '}
        <strong>NBFM</strong>, η πιο συχνή λανθασμένη απάντηση.
      </p>
    ),
  },
