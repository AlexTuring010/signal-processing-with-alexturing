    ...
    >
      ...
      <ol className="my-3 list-none space-y-1.5 px-4">
        {order.map((stepIdx, position) => {
          const correct = checked ? stepIdx === position : null
          return (
            <li
              key={stepIdx}
              className={cn(
      ...
      <div className="flex flex-wrap items-center gap-2 px-4 pb-4">
        <button
          type="button"
          onClick={check}
          className="inline-flex items-center gap-1.5 rounded-md border border-accent/40 bg-accent-soft/40 px-3 py-1.5 text-sm font-medium transition hover:bg-accent-soft/60"
        >
